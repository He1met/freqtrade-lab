...
单命令负责固定URL获取、有限HTML文本提取、一次无工具Codex提案、已有离线预筛及收据。当前只完成工程和合成验证；真实HTTP/provider等待监督审固定头后另行裁定。不创建调度器，不写Generation或数据库，不运行市场研究。两个已知BIS/MIT页面用于provider集成验证，不当作新发现或独立证据。
...
- 固定CLI 0.153.4二进制身份；复用项目已存在的shell_tool主开关和工具feature关闭机制，同时关闭apps/plugins/MCP关联、web、code mode、browser/computer/image、hooks、multi-agent、goals和auth elicitation等40个开关；启用skip_host_skill_discovery。exec忽略用户config/rules，隔离空workspace，mcp_servers空配置，web_search disabled。`unified_exec`部署控制仍true，不能将其报告false；模型可见shell主开关实际probe false。只读sandbox是额外限制，不是无工具证明。
- 本地无模型feature probe验证40个required false、skip_host_skill_discovery true。实际运行前再次核二进制与feature输出；任何不符BLOCKED_TOOL_ISOLATION。依据是固定版本的能力开关约束，并非仅prompt禁止；CLI/API内部最终tool inventory未通过真实请求核验，审阅时必须保留这一限制。JSONL再拒绝工具/未知事件是事后完整性防线，不能代替前述开关。现有Generation输出schema/DB生命周期不被复用。
- 固定请求/stream retries=0并关闭unbounded_connection_retries，命令不重试。计数是一次CLI provider进程预算，不伪称已拿到供应方计费明细。模型选择gpt-5.6-sol用于该有限提案试运行，实际可用性未验证；不自动切模型。
