...
from lab import perp_data_candidate, perp_schedule


def run(root, policy_path, updater=None):
    root = Path(root)
    policy, policy_sha = perp_schedule.load_policy(policy_path)
    if updater is None:
        updater = lambda path: perp_data_candidate.update(path,candidate_required=bool(policy.get('data_priority')))
    ...
    with (root/'heartbeat.lock').open('a') as lock:
        ...
        perp_schedule.tick(root/'scheduler', policy_path)
        # Recheck inside the same lock used by research claim, and retain it for
        # the bounded acquisition. A heartbeat lock alone cannot exclude claim.
        with perp_schedule.locked(root/'scheduler', policy_path) as (_, _, current):
            active = [t['id'] for t in current['tasks'] if t['status'] in {'RUNNING', 'UNKNOWN_INTERRUPTED'}]
            ...
            result = dict(status='HEARTBEAT_CHECKPOINT', observed_at=observed,
                ...
                research=[dict(id=t['id'],status=t['status'],hypothesis=t['hypothesis'])
                          for t in current['tasks'] if t['status'] in perp_schedule.OPEN],
                due=current['due'], native_calls=0, production_orders=0,
