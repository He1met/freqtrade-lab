...
assert context['state']['status'] == 'SEARCH_TERMINATED_NO_FINALIST'
...
with zipfile.ZipFile(archive) as z:
    ...
    assert len(names) == 1
    report = json.loads(z.read(names[0]))['strategy']['BchPriorCloseTrend28']
    embedded = [n for n in z.namelist() if n.endswith('_BchPriorCloseTrend28.py')]
    assert len(embedded) == 1 and sha(z.read(embedded[0])) == sha((r/'BchPriorCloseTrend28.py').read_bytes())
...
blocks = ['2023-11-13','2024-01-01','2024-02-19','2024-04-08','2024-05-27','2024-07-15']
block_report=[]
details=[]
...
without_best = totals['conservative_net_usdt']-max(0,max(d['conservative_net_usdt'] for d in details))
...
check('core_total_trades',len(trades),'>=8',len(trades)>=8)
...
check('conservative_MTM_DD_pct',audit['conservative_mtm_drawdown_pct'],'<=15',audit['conservative_mtm_drawdown_pct']<=15)
check('native_DD_pct',result['max_drawdown_pct'],'<=15',result['max_drawdown_pct']<=15)
check('mean_holding_minutes',result['average_holding_period_minutes'],'>=4320',result['average_holding_period_minutes']>=4320)
...
check('effective_natural_samples',len(effective),'>=8',len(effective)>=8)
check('effective_long_samples',natural_longs,'>=2',natural_longs>=2)
check('effective_short_samples',natural_shorts,'>=2',natural_shorts>=2)
active_blocks=sum(b['effective_natural_trades']>0 for b in block_report)
check('blocks_with_natural_sample',active_blocks,'>=3 of5',active_blocks>=3)
...
out={'schema':'issue96-search-protocol-review-v1','reviewed_at_utc':datetime.now(timezone.utc).isoformat(),'project_status':context['state']['status'],
     'economic_classification':'BOUNDED_NEGATIVE','sample_classification':'UNDERPOWERED_NO_EVIDENCE',
     'all_protocol_gates':'FAILED','actual_Search_attempts':1,'campaign_id':context['state']['campaign_id'],
     'candidate_id':attempt['candidate_id'],'protocol_sha256':sha((r/'frozen-protocol.md').read_bytes()),'strategy_sha256':attempt['strategy_sha256'],
     ...
     'ResearchRun_id':None,'D':'QC_ONLY_NOT_EXECUTED','H_Stress':'SEALED_UNREAD_UNACQUIRED','no_finalist_no_further_attempt':True,
