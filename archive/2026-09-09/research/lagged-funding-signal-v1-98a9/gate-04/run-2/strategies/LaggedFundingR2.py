...
class LaggedFundingR2(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "5m"
    can_short = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(dataframe["date"].dt.hour == 0) & (dataframe["date"].dt.minute == 0) & dataframe["price_valid"] & dataframe["time_valid"] & dataframe["funding_valid"] & (dataframe["return_24h"] <= -0.02) & (dataframe["lagged_funding"] > 0.0001), "enter_short"] = 1
        return dataframe
