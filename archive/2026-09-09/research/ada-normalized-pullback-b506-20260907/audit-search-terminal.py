...
sys.path.insert(0, '/Users/shenjianpeng/.codex/worktrees/b506/freqtrade-lab')
...
context = json.load(urllib.request.urlopen('http://127.0.0.1:8799/api/search/context'))
...
with zipfile.ZipFile(archive) as z:
    ...
    assert len(names) == 1
    report = json.loads(z.read(names[0]))['strategy']['AdaNormalizedTrendPullback3D']
    embedded = [n for n in z.namelist() if n.endswith('_AdaNormalizedTrendPullback3D.py')]
    assert len(embedded) == 1 and sha(z.read(embedded[0])) == sha((r/'AdaNormalizedTrendPullback3D.py').read_bytes())
...
out={'schema':'issue101-search-protocol-review-v1','reviewed_at_utc':datetime.now(timezone.utc).isoformat(),'project_status':context['state']['status'],
