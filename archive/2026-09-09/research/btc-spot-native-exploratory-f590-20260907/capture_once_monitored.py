...
assert json.loads((ROOT/'synthetic-02-execution-receipt.json').read_text())['status']=='SYNTHETIC_02_PASS'
assert not (ROOT/'source-2024-01').exists()
command=json.loads((ROOT/'capture-command.json').read_text())['argv']
...
def monitored_guard(exchange):
    ...
    def request(method,url,*args,**kwargs):
        ...
        assert q.get('instId')==['BTC-USDT']
        if p.path.endswith('/instruments'):
            assert q.get('instType')==['SPOT'] and state['http_calls']==0
        else:
            assert q.get('bar')==['1Dutc']
            assert int(q['before'][0])>=1704067200000-1
            assert int(q['after'][0])<=1735603200000
        assert state['http_calls']<5, 'authorized HTTP call count exceeded'
        state['http_calls']+=1
...
pre=append({'record_type':'AUTHORIZED_OBSERVED_TRAINING_CAPTURE_START','pair':'BTC/USDT','source_window':['2024-01-01','2024-12-31'],
    'maximum_http_calls':5,'maximum_seconds':900,'maximum_bytes_response_after':50*1024**2,'capture_invocations':1,
    'market_native_calls':0,'known_seen_training':True},'8ad052db5aaafdef5edb832e1b2b39c603a6964f14f27dd84a87239abc0bb009')
...
try:
    ...
    receipt={'status':'CAPTURE_PASS_PENDING_SOURCE_QC' if result_code==0 else 'CAPTURE_FAILED_STOP',
        ...
        'monitoring':'maximum 5 before-request calls; 900s signal watchdog; response-after and final-write 50MiB checks',
        'wire_call_counter':'requests.session.request observed calls','retry_allowed':False}
    put('capture-execution-receipt.json',receipt)
    receipt['ledger']=append({'record_type':'OBSERVED_TRAINING_CAPTURE_TERMINAL',**receipt})
