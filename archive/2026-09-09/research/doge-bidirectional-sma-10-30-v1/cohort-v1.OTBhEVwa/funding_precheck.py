...
assert freeze['state']=='OPEN' and freeze['number']==65
...
p.SYMBOL='DOGE/USDT:USDT';p.INSTRUMENT_ID='DOGE-USDT-SWAP';p.PAIR_FAMILY='DOGE-USDT';p.FUTURES_TIMEFRAME='1d'
p.PROFILE_ACQUISITION={'scope':'FUNDING_ONLY_PREFLIGHT','issue':65,'pair':p.SYMBOL,'timeframe':'1d'}
...
try:
    ...
    for year,month in p._archive_months():
        ...
        if (year,month)==(2024,2): assert len(month_expected)==86
        if (year,month)==(2026,2): assert len(month_expected)==1
