...
class ConsolidationChannelR1(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1d"
    can_short = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[(((dataframe["valid_close"] > 0) & (dataframe["valid_volume"] > 0)) & (dataframe["close"] > dataframe["upper"])), "enter_long"] = 1
        dataframe.loc[(((dataframe["valid_close"] > 0) & (dataframe["valid_volume"] > 0)) & (dataframe["close"] < dataframe["lower"])), "enter_short"] = 1
        return dataframe
