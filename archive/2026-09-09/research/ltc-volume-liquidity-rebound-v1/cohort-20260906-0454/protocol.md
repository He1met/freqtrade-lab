# Issue #93 冻结协议：LTC_VOLUME_LIQUIDITY_REBOUND_V1

**状态：授权冻结与一次S+D采集QC；Search仍未授权。** 建议用 OKX `LTC/USDT` 现货日线，检验“异常放量的显著下跌可能包含短期流动性抛售，下一开盘承担风险，持有两天获得补偿”。只推荐这一条完整规则、一轮一次 Search。当前盈利、真实样本量和成交量增量贡献均 UNKNOWN。
...
上述只需静态算术（平方写作乘法）、`shift/rolling/mean/min`和三个populate方法，40根前史覆盖依赖链；不需要custom_exit或新指标库。值前准备将对最终源码运行静态validator与纯合成样例（不运行native backtest）验证事件在t+1入场/t+3退出、相邻冲击去重和同bar止损，不许以测试为由读取行情。
...
## Issue #93 授权与会计语义补充（与上文一起冻结）

监督任务01a05dcc-17fd-7972-9177-9fed95e4b07a于本任务授权创建Issue93、最小合成因果检查、批准注册、值前冻结与唯一一次采集QC。上文选择阶段的未执行描述是历史状态；本补充将其限定为本次准备，数值门与交易规则不变。执行checkout仅3c7c，已确认干净后切到7ae2b6b6c45cfb57c40a13dccd697ce1c57d08a4；缺少对象时仅fetch指定commit，没有改变其他checkout。项目业务代码、native、Schema预算0。所有脚本Git外。唯一策略为LtcVolumeLiquidityReboundV1.py，最终冻结以该字节SHA为准。

跳空补差：首先读同一交易的原生实际退出价格；若原生已按更差开盘退出，仅核验，不重复扣跳空。仅当既有long仓位原生stop退出价确实高于该日open时，对原生已经实现的现金再减q*(native_exit-open)，不再减第二遍从理想stop到open的全额。新仓同bar止损不把本bar开盘误当持仓隔夜跳空；此补差是同路径保守账务审计，不改native回放。费与滑点按实际/调整后名义额逐项标明。

MTM准备：现金只记录真实买卖及手续费/滑点/必要的未反映跳空补差。未平仓预计清算费是权益估值准备：equity=cash+q*close-reserve；它不重复写现金。真实退出后现金收到净退出款，仓位归零且reserve释放。订单同时间必须按trade_id/order_id及trade内因果依赖处理；新仓同bar止损必须先buy后sell。不得复用Issue92中按全局sell优先的旧helper。原生backtest_loop确实先处理entry订单，再检查退出并处理exit订单；仅用提取的纯方法及stub合成验证，不额外native smoke。

固定30日块只作集中度计数；跨块持仓整笔利润归入场块，不拆单。统计CI保留完整逐日收益序列（含零仓日及跨块持仓日收益），按原固定块长/种子重采样，不把块收益或交易收益替代日序列。

批准状态由现有review_generation明确记录。SQLite用单一只读事务中Profile、批准候选（包含代码、批准状态及Generation身份）和六表计数的逻辑快照及其SHA绑定；不以可能忽略WAL的主DB文件SHA作为授权基础。关闭事务后正式冻结；首次网络前再次核验相同逻辑快照、所有冻结文件与native/producer字节。

本授权仅准备与一次采集。此次API或QC失败即保留回执并停止；source成功只prepare-search-data和隔离端口Console只读GET。禁止Search POST/screen-search/native、D/H/Stress/finalist导入及Release。完成后Issue保持OPEN，由监督另审唯一一轮一次Search。
