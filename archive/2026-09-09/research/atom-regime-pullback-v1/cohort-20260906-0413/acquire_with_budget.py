"""Issue92-only execution protection around the unchanged Profile producer.
...
def bounded_requests(log, *, seconds=MAX_SECONDS):
    ...
    def guarded(session, method, url, *args, **kwargs):
        ...
        if endpoint.path == '/api/v5/public/instruments':
            if query != {'instType':['SPOT'],'instId':['ATOM-USDT']}:
                raise BudgetStop('SOURCE_INSTRUMENT_REJECTED')
        else:
            if set(query) != {'instId','bar','limit','before','after'} or query['instId'] != ['ATOM-USDT'] or query['bar'] != ['1Dutc']:
            ...
            try:
                ...
                start = int(datetime(2021,2,13,tzinfo=timezone.utc).timestamp()*1000)
    ...
    try:
        with tempfile.TemporaryFile() as log, requests.Session() as session:
            with bounded_requests(log) as state:
                for _ in range(24):
                    try:
                        session.request('GET', 'https://www.okx.com/api/v5/public/instruments?instType=SPOT&instId=ATOM-USDT', allow_redirects=False)
...
def acquire():
    ...
    sys.argv = ['fetch_okx_profile_data.py', '--output-root', str(ROOT/'source'),
                ...
                '--window-spec', str(ROOT/'window-spec.json'), '--pre-roll-candles', '200',
