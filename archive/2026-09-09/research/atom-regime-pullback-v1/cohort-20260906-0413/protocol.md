# ATOM_REGIME_PULLBACK_V1 — Issue92 值前冻结协议

2026-09-06；代码基线 `0e4d8e9d33b806239bb13d618b4db2881a892ac3`。**唯一推荐：OKX ATOM/USDT 现货日线，上升状态中的回撤回归；一次固定基线 Search，1 轮/1 attempt。** 本协议经监督批准用于 Issue92。当前仅授权登记、一次 S+D 采集及QC；Search/native/D/H/finalist导入尚未授权。本轮仅 3 次官方文档/身份请求，全部成功、无重试；没有市场值、策略入库、项目代码/数据库修改或 native 调用。
...
3. **风险：**native DD≤20%，同时每日 cost-MTM 从 E0=1000 开始，权益=现金+未平仓基础币数量×该 UTC 日收盘价，计算历史峰值相对 DD≤20%。现金在每次实际 native 入场时扣买入名义额及原生开仓费，在实际退出时加卖出名义额并扣原生平仓费；逐腿额外滑点在该成交时刻作为现金成本扣除，不重写成交价。未平仓按日收盘盯市；日内实际现金事件按时间排序，期末强平独立标识并计入最终现金和费用，同一天在收盘前已退出的头寸不重复计市值。基本与敏感性现金分别重建，原生成交数量/时点固定；不得复用闭合交易DD代替盯市DD。净收益决定性负时可早停昂贵指标并保留NULL。日线风险估计保留无法重建日内成交路径的局限；不能把价格止损当亏损上限。不能核对账务/现金则技术阻塞，不填零。
4. **样本/集中度：**每阶段至少 12 个自然完成交易；S 至少 8 个、D/H 各至少 6 个完整 30 日暴露组。每组从尚未归组的第一个入场起算。组结束为起点+30日与该组全部交易最终退出时间的较晚者；组期间及延伸期间所有入场归同组，并递归更新结束时间，延伸期不同时启动新组。下一组从上一组结束后严格较晚的首个入场开始；恰好同刻入场仍归上一组。阶段末未完整覆盖组期、或组内持仓未自然完成，不计完整组；期末强平独立标识，不算自然交易，不拆长仓凑独立样本。移除收益最高组后阶段成本后净收益仍 >0。组数不是 IID 证明；即使全过，也只称本协议研究通过，不称长期盈利已证明。净正但样本不足为 `UNDERPOWERED`，不得降门。
