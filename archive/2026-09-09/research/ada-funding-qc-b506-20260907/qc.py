...
def alarm_handler(*_): raise TimeoutError('WALL_BUDGET')
signal.signal(signal.SIGALRM,alarm_handler)
signal.alarm(600)
started=now(); tick=time.monotonic(); receipts=[]; total=0; expected_start=LO
write('authorization.json',{'supervisor':'01a05dcc-17fd-7972-9177-9fed95e4b07a','issue':101,
 'authorized_phase':'ADA S funding-only QC','start_ms':LO,'end_exclusive_ms':HI,
 ...
 'proposal_sha256':hashlib.sha256(Path('/Users/shenjianpeng/.codex/worktrees/b506/freqtrade-lab/docs/next-normalized-pullback-feasibility.md').read_bytes()).hexdigest(),
...
def guarded(method,url,**kwargs):
    ...
    if not receipts: assert p.path=='/fapi/v1/exchangeInfo' and not q
        ...
        assert q=={'symbol':['ADAUSDT'],'startTime':[str(expected_start)],'endTime':[str(HI-1)],'limit':['1000']}
...
try:
    ...
    matches=[x for x in info['symbols'] if x.get('symbol')=='ADAUSDT']
    assert len(matches)==1
    m=matches[0]
    report['market']={k:m.get(k) for k in ['symbol','status','contractType','baseAsset','quoteAsset','marginAsset']}
    assert report['market']=={'symbol':'ADAUSDT','status':'TRADING','contractType':'PERPETUAL','baseAsset':'ADA','quoteAsset':'USDT','marginAsset':'USDT'}
    rows=[]
    for page in range(2):
        batch=exchange.fapiPublicGetFundingRate({'symbol':'ADAUSDT','startTime':expected_start,'endTime':HI-1,'limit':1000})
    ...
    report['qc']={'expected_events':len(expected),'actual_events':len(rows),
      ...
      'wrong_symbol_count':sum(x.get('symbol')!='ADAUSDT' for x in rows),
    ...
    (ROOT/'private-error.txt').write_text(repr(exc))
    report['failure_type']=type(exc).__name__
    report['failure']='REQUEST_OR_METADATA_VALIDATION_FAILED' # Do not expose CCXT raw-response exception text.
finally:
    signal.alarm(0)
    report['byte_budget_semantics']='Retained application bytes <=2MiB; decoded iterator can yield one extra <=4096-byte chunk on crossing, immediately stops; not a wire-byte cap'
