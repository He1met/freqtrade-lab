...
for stage,relative,start,end,fstart,expected in [
 ('S+D','complete-source','2023-09-30','2025-11-03','2023-11-06',(765,18360,2184)),
 ('S','search-campaign/acquisition','2023-09-30','2024-11-04','2023-11-06',(401,9624,1092)),
 ('D_QC_ONLY','development-pilot/development-isolation','2024-09-28','2025-11-03','2024-11-04',(401,9624,1092))]:
 root=r/relative
 p=json.loads((root/'retained-data-provenance.json').read_bytes())
 assert p['source']['pair']=='DOGE/USDT:USDT' and p['source']['instrument_id']=='DOGEUSDT'
 ...
 for suffix,freq,n in [('1d-futures','1D',expected[0]),('1h-mark','1h',expected[1])]:
  path=root/'data/binance/futures'/('DOGE_USDT_USDT-'+suffix+'.feather')
