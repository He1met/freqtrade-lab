...
ROOT=Path(__file__).resolve().parent/'metadata-probe';ROOT.mkdir(exist_ok=False)
...
for name,url,lo in requests:
 ...
 try:
  ...
  if lo is None:
   ...
   if timestamps!=[lo,lo+28800000,lo+57600000]:raise ValueError('FUNDING_CALENDAR_MISMATCH')
  row['status']='PASS'
