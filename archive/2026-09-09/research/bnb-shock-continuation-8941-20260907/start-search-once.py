...
assert context['candidates'][0]['strategy_sha256']==sha((r/'BnbDailyShockContinuation48H.py').read_bytes())
assert json.loads((r/'source-aggregation-qc.json').read_bytes())['status']=='SOURCE_QC_PASS'
assert json.loads((r/'signal-capacity.json').read_bytes())['status']=='CAPACITY_UPPER_BOUND_POSSIBLE'
record=dict(record_type='SEARCH_REGISTERED_PRE_RUN',cohort_id='issue104-bnb-daily-shock-continuation-single-v1',recorded_at_utc=datetime.now(timezone.utc).isoformat(),issue=104,maximum_actual_native_Search_runs=1,source=json.loads((r/'source-publication-v2.json').read_bytes()),root=str(r),candidate_id=context['candidates'][0]['candidate_id'],strategy_sha256=context['candidates'][0]['strategy_sha256'],protocol_sha256=sha((r/'final-protocol.md').read_bytes()),D_status='PHYSICALLY_ISOLATED_QC_ONLY_NO_STRATEGY_RUN',H_Stress='OHLCV_SEALED_LIMITED_FUNDING_METADATA_QC')
...
with ledger.with_suffix(ledger.suffix+'.lock').open('a+b') as lock:
 ...
 assert sha(before)==json.loads((r/'ledger-capacity-receipt.json').read_bytes())['after_sha256']
...
payload={'profile_id':'issue104-bnb-daily-shock-continuation-48h-v1','candidate_ids':[context['candidates'][0]['candidate_id']]}
