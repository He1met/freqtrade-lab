...
class ShockReversalR1(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "5m"
    can_short = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[qtpylib.crossed_below(dataframe["shock"], -0.02) & (dataframe["close"] < dataframe["prior_price_mean"] * 0.985) & (dataframe["valid_low"] > 0) & (dataframe["volume"] > 0), "enter_long"] = 1
        dataframe.loc[qtpylib.crossed_above(dataframe["shock"], 0.02) & (dataframe["close"] > dataframe["prior_price_mean"] * 1.015) & (dataframe["valid_low"] > 0) & (dataframe["volume"] > 0), "enter_short"] = 1
        return dataframe
