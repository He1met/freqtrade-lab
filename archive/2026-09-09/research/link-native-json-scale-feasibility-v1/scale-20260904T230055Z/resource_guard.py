"""Ephemeral bounded Gate owned-child watchdog. RSS measurement, not a backtest runner."""
...
DEADLINE = datetime.datetime(2026, 9, 5, 0, 30, 55, tzinfo=datetime.timezone.utc).timestamp()

def run(label, seconds, limit, argv):
    if time.time() >= DEADLINE:
        raise RuntimeError("Gate 90-minute deadline reached")
    ...
    child = subprocess.Popen(argv, stdout=stdout, stderr=stderr, start_new_session=True,
                             env={**os.environ, "PYTHONDONTWRITEBYTECODE": "1", "TMPDIR": str(ROOT / "tmp"), "PYTHONPATH": "/Users/shenjianpeng/.codex/runs/freqtrade-lab/issue-43-profile-driven-v1/freqtrade"})
