...
For a new cohort, Development and future Holdout use two independent,
create-exclusive roots. First acquire only the already closed Development
window. This source contract has no Holdout field and contains exactly two
startup hours plus 60 Development days:

The dates below illustrate the byte-level contract only. They do not authorize
replaying a consumed Search window, retroactive finalist discovery, or treating
the Development close as a new discovery date; a real cohort must bind its own
previously frozen Search finalist and naturally separated rolling windows.

```json
{
  "schema": "freqtrade-lab-okx-development-window-v1",
  "data_start_utc": "2026-07-30T22:00:00Z",
  "development_start_utc": "2026-07-31T00:00:00Z",
  "end_exclusive_utc": "2026-09-29T00:00:00Z"
}
```

In this rolling example, `2026-09-29T00:00:00Z` is the Development close, not
a discovery start. Only after that close may the public acquisition helper run
with the reviewed window file. It requires no credentials and its output
remains local:

```bash
PYTHONDONTWRITEBYTECODE=1 /absolute/private/path/freqtrade-2026.7-venv/bin/python \
  tests/fixtures/freqtrade_2026_7/producer/fetch_okx_public_data.py \
  --output-root /absolute/private/path/development-source/acquisition \
  --strategy-file /absolute/private/path/SearchFinalist.py \
  --research-spec /absolute/private/path/SearchFinalist-spec.json \
  --window-spec /absolute/private/path/development-window.json
```

Review the printed provenance and retrieval-receipt hashes, then publish the
immutable Development root. This step pre-binds one Candidate, Profile, and
canonical UUIDv4 ResearchRun and copies no source plan, Holdout, or Stress data:

```bash
PYTHONDONTWRITEBYTECODE=1 /absolute/private/path/freqtrade-2026.7-venv/bin/python \
  scripts/run_bounded_research_pilot.py prepare-development-root \
  --source-root /absolute/private/path/development-source/acquisition \
  --source-provenance-sha256 <64-lowercase-hex> \
  --source-receipt-sha256 <64-lowercase-hex> \
  --output-root /absolute/private/path/development-root \
  ...
  --candidate-id <approved-candidate-id> \
  --research-run-id <new-uuid>
```

Only after that Development passes does the same transaction freeze the future
Holdout timerange: the next UTC day after the Gate pass, for the Profile's fixed
`holdout_days`. The date is not a discovery input. After that future window has
naturally closed, publish a second root from a new reviewed public acquisition:

```bash
PYTHONDONTWRITEBYTECODE=1 /absolute/private/path/freqtrade-2026.7-venv/bin/python \
  scripts/run_bounded_research_pilot.py prepare-holdout-root \
  --source-pilot-root /absolute/private/path/future-complete-source-pilot \
  --source-provenance-sha256 <64-lowercase-hex> \
  --source-receipt-sha256 <64-lowercase-hex> \
  --output-root /absolute/private/path/holdout-root \
  --development-root /absolute/private/path/development-root \
  --freqtrade-python /absolute/private/path/freqtrade-2026.7-venv/bin/python \
  --freqtrade-source /absolute/private/path/clean-freqtrade-2026.7 \
  --database /absolute/private/path/freqtrade-lab-workspace/lab.sqlite \
  --research-run-id <same-new-uuid>
```

The future adapter verifies the trusted receipt, closedness, exact three public
series, UTC continuity, and the frozen window, then publishes only the Holdout
bytes plus two startup hours. It binds both root hashes, the same Run/Candidate/
Profile, the Development input manifest, and the Development Artifact. Stress
reuses those exact Holdout bytes with the frozen fee multiplier. No schema-v1
table, Hyperopt path, retry, or threshold override is added.
...
python3 scripts/serve_research_console.py \
  ...
  --development-root /absolute/private/path/development-root \
  --holdout-root /absolute/private/path/holdout-root \
...
`--holdout-root` is omitted until the frozen future window closes; Development
remains available while Holdout stays `BLOCKED_DATA / SEALED_UNREAD`.
`--pilot-root` remains only as the legacy combined-root alias for historical
fixtures and prior runs.
...
#### Development-first rolling window

The command above without `--window-spec` preserves the short legacy fixture
workflow. New research uses `freqtrade-lab-okx-development-window-v1`, shown in
the Development-root section above. It deliberately has no Holdout boundary,
so a naturally closed 60-day Development period can run without waiting for or
reading a future 30-day Holdout.

`freqtrade-lab-okx-window-v2` remains supported only for already frozen legacy
combined Pilot inputs. It is not the discovery or acquisition contract for a
new split-root cohort. Neither source format selects a Candidate, passes
Development, authorizes Holdout, establishes a verdict, or proves
