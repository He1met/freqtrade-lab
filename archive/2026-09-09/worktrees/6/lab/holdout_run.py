...
HOLDOUT_AUTHORIZATION_SCHEMA_V2 = "freqtrade-lab-holdout-authorization-v2"
...
HOLDOUT_ROOT_SCHEMA = "freqtrade-lab-holdout-root-v1"
HOLDOUT_ROOT_MANIFEST = "holdout-root.json"
...
class FrozenHoldoutCapability:
    ...
    separate_root: bool = False
    root_schema: Optional[str] = None
    bound_research_run_id: Optional[str] = None
    bound_candidate_id: Optional[str] = None
    bound_generation_run_id: Optional[str] = None
    bound_candidate_sha256: Optional[str] = None
    bound_profile_id: Optional[str] = None
    bound_profile_sha256: Optional[str] = None
    development_root_sha256: Optional[str] = None
    development_input_manifest_sha256: Optional[str] = None
    development_artifact_receipts: Tuple[Tuple[str, str], ...] = ()
    ...
    def public(self) -> dict[str, Any]:
        return {
            ...
            "one_shot": True,
            "data_root_mode": "SEPARATE" if self.separate_root else "LEGACY_COMBINED",
        }
...
def _freeze_staged_holdout_root(
    root: Path,
    development: FrozenDevelopmentCapability,
) -> FrozenHoldoutCapability:
    """Freeze a future Holdout-only root without opening its market bytes."""
    manifest_bytes = _read_regular(
        root / HOLDOUT_ROOT_MANIFEST,
        "Holdout root manifest",
        2 * 1024 * 1024,
    )
    manifest = _json_bytes(manifest_bytes, "Holdout root manifest")
    required = {
        "schema",
        "binding",
        "holdout_timerange",
    ...
    if set(manifest) != required or manifest.get("schema") != HOLDOUT_ROOT_SCHEMA:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root manifest is invalid")
    ...
    binding_keys = {
        ...
        "development_root_manifest_sha256",
        "development_input_manifest_sha256",
        "development_artifact",
    }
    if not isinstance(binding, dict) or set(binding) != binding_keys:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root binding is invalid")
    for name in (
        "research_run_id",
        "candidate_id",
        "generation_run_id",
    ...
    for name in (
        ...
        if not isinstance(binding.get(name), str) or _SHA256.fullmatch(binding[name]) is None:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root binding is invalid")
    artifact = binding.get("development_artifact")
    if not isinstance(artifact, dict) or set(artifact) != {
        "archive_sha256",
        "metadata_sha256",
        "provenance_sha256",
    }:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root artifact binding is invalid")
    for value in artifact.values():
        if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root artifact binding is invalid")
    if (
        binding["research_run_id"] != development.bound_research_run_id
        or binding["candidate_id"] != development.bound_candidate_id
        or binding["generation_run_id"] != development.bound_generation_run_id
        or binding["candidate_code_sha256"] != development.bound_candidate_sha256
        or binding["research_profile_id"] != development.bound_profile_id
        or binding["profile_binding_sha256"] != development.bound_profile_sha256
        or binding["development_root_manifest_sha256"] != development.plan_sha256
    ):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root disagrees with Development root")

    start, stop = _timerange(manifest.get("holdout_timerange"))
    if stop - start != timedelta(days=30):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root must contain exactly 30 days")
    try:
        multiplier = float(manifest.get("stress_fee_multiplier"))
    except (TypeError, ValueError, OverflowError) as exc:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root stress multiplier is invalid") from exc
    if not math.isfinite(multiplier) or multiplier <= 1.0:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root stress multiplier is invalid")
    if manifest.get("policy") != {
        ...
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root policy is invalid")
    if manifest.get("freqtrade") != {
        ...
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root runtime contract mismatch")
    source = manifest.get("source")
    if (
        not isinstance(source, dict)
        or set(source) != {"host", "authentication", "pair", "instrument_id"}
        ...
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root market identity mismatch")
    ...
    if not isinstance(source_receipts, dict) or set(source_receipts) != {
        ...
        raise HoldoutRunError("BLOCKED_DATA", "Holdout source receipt is invalid")
    for name in ("source_provenance_sha256", "retrieval_receipt_sha256"):
        value = source_receipts.get(name)
        if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout source receipt is invalid")
    try:
        source_start = datetime.fromisoformat(
            str(source_receipts.get("source_start_utc")).replace("Z", "+00:00")
        )
        source_stop = datetime.fromisoformat(
            str(source_receipts.get("source_end_exclusive_utc")).replace(
                "Z", "+00:00"
            )
        )
        retrieved = datetime.fromisoformat(
            str(source_receipts.get("retrieval_finished_at_utc")).replace(
                "Z", "+00:00"
            )
        )
    except ValueError as exc:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout source receipt is invalid") from exc
    if (
        source_receipts.get("fully_closed_at_fetch") is not True
        or any(
            value.utcoffset() != timedelta(0)
            for value in (source_start, source_stop, retrieved)
        )
        or source_start > start - timedelta(hours=2)
        or source_stop < stop
        or retrieved < stop
    ):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout source is not fully closed")

    files = manifest.get("files")
    if not isinstance(files, dict) or not files:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root file receipt is invalid")
    expected_roles = {
        "config.json": "config",
        "market_snapshot.json": "market_snapshot",
        "isolated_tiers_snapshot.json": "leverage_tiers",
    }
    config_record: Optional[Tuple[int, str, Optional[str]]] = None
    receipts: list[Tuple[str, int, str, Optional[str]]] = []
    seen: set[str] = set()
    for name, raw in files.items():
        relative = _safe_relative(name, "Holdout root input")
        if not isinstance(raw, dict) or set(raw) != {"bytes", "sha256", "role"}:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root file receipt is invalid")
        size, digest, role = _receipt_record(raw, f"Holdout root input {name!r}")
        expected_role = expected_roles.get(str(name))
        if expected_role is not None:
            if role != expected_role:
                raise HoldoutRunError("BLOCKED_DATA", "Holdout root file role is invalid")
        elif isinstance(name, str) and name.startswith("data/okx/"):
            if role != "market_data":
                raise HoldoutRunError("BLOCKED_DATA", "Holdout root file role is invalid")
        else:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root has an extra input")
        candidate = root / relative
        try:
            info = candidate.stat(follow_symlinks=False)
        except OSError as exc:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root input is unavailable") from exc
        if candidate.is_symlink() or not stat.S_ISREG(info.st_mode) or info.st_size != size:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout root input envelope mismatch")
        seen.add(str(name))
        if name == "config.json":
            config_record = (size, digest, role)
        else:
            receipts.append((str(name), size, digest, role))
    try:
        actual = {
            path.relative_to(root).as_posix()
            for path in root.rglob("*")
            if path.is_file() and path.name != HOLDOUT_ROOT_MANIFEST
        }
        if any(path.is_symlink() for path in root.rglob("*")):
            raise OSError("symlink")
    except (OSError, RuntimeError, ValueError) as exc:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root layout is unsafe") from exc
    if actual != seen or config_record is None or not any(
        name.startswith("data/okx/") for name, *_ in receipts
    ):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root file set is invalid")
    config = _read_regular(root / "config.json", "Holdout root config")
    if (
        len(config) != config_record[0]
        or hashlib.sha256(config).hexdigest() != config_record[1]
        or config_record[1] != development.config_sha256
    ):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout root config receipt mismatch")
    runner = _read_regular(DEFAULT_RUNNER, "backtest runner", 2 * 1024 * 1024)
    runner_sha = hashlib.sha256(runner).hexdigest()
    if runner_sha != development.runner_sha256:
        raise HoldoutRunError("BLOCKED_DATA", "Holdout runner drifted from Development")
    root_info = os.stat(root)
    return FrozenHoldoutCapability(
        status="READY",
        reason="future Holdout-only inputs are frozen and remain sealed until authorization",
        ...
        acquisition_provenance_sha256=str(
            source_receipts["source_provenance_sha256"]
        ),
        config_sha256=config_record[1],
        runner_sha256=runner_sha,
        development_timerange=development.development_timerange,
        holdout_timerange=str(manifest["holdout_timerange"]),
        stress_fee_multiplier=multiplier,
        pair=str(source["pair"]),
        local_receipts=tuple(sorted(receipts)),
        root_identity=(root_info.st_dev, root_info.st_ino),
        separate_root=True,
        root_schema=HOLDOUT_ROOT_SCHEMA,
        bound_research_run_id=str(binding["research_run_id"]),
        bound_candidate_id=str(binding["candidate_id"]),
        bound_generation_run_id=str(binding["generation_run_id"]),
        bound_candidate_sha256=str(binding["candidate_code_sha256"]),
        bound_profile_id=str(binding["research_profile_id"]),
        bound_profile_sha256=str(binding["profile_binding_sha256"]),
        development_root_sha256=str(binding["development_root_manifest_sha256"]),
        development_input_manifest_sha256=str(
            binding["development_input_manifest_sha256"]
        ),
        development_artifact_receipts=tuple(sorted(artifact.items())),
    )
...
def freeze_holdout_capability(
    ...
    *,
    development_capability: Optional[FrozenDevelopmentCapability] = None,
) -> FrozenHoldoutCapability:
    """Freeze later-phase receipt metadata without reading retained market values."""
    separate_root = development_capability is not None
    development = development_capability or freeze_development_capability(
        pilot_root, freqtrade_python, freqtrade_source
    )
    try:
        if development.status != "READY" or development.pilot_root is None:
            raise HoldoutRunError("BLOCKED_DATA", development.reason)
        if separate_root:
            _require_development_ready(development)
            pilot = _resolve_directory(pilot_root, "Holdout root")
            development_info = os.stat(development.pilot_root)
            holdout_info = os.stat(pilot)
            if (development_info.st_dev, development_info.st_ino) == (
                holdout_info.st_dev,
                holdout_info.st_ino,
            ):
                raise HoldoutRunError(
                    "BLOCKED_DATA", "Development and Holdout roots must be independent"
                )
            try:
                pilot.relative_to(development.pilot_root)
            except ValueError:
                pass
            else:
                raise HoldoutRunError(
                    "BLOCKED_DATA", "Holdout root cannot be inside Development root"
                )
            try:
                development.pilot_root.relative_to(pilot)
            except ValueError:
                pass
            else:
                raise HoldoutRunError(
                    "BLOCKED_DATA", "Development root cannot be inside Holdout root"
                )
            manifest_path = pilot / HOLDOUT_ROOT_MANIFEST
            if manifest_path.exists() or manifest_path.is_symlink():
                return _freeze_staged_holdout_root(pilot, development)
            raise HoldoutRunError(
                "BLOCKED_DATA", "separate Holdout root manifest is unavailable"
            )
        else:
        ...
        if separate_root:
            development_start, development_stop, _ = _development_window(
        ...
        if (
            ...
            or development_timerange != development.development_timerange
        ...
        config_size, config_digest, _ = _receipt_record(
            tracked["config.json"], "Pilot config"
        )
        if (
            len(config_bytes) != config_size
            or hashlib.sha256(config_bytes).hexdigest() != config_digest
            or (separate_root and config_digest != development.config_sha256)
        ):
            raise HoldoutRunError("BLOCKED_DATA", "Holdout config receipt mismatch")
        ...
        return FrozenHoldoutCapability(
            ...
            pair=str(source["pair"]),
            local_receipts=tuple(sorted(receipts)),
            root_identity=(holdout_info.st_dev, holdout_info.st_ino),
            separate_root=separate_root,
            root_schema="freqtrade-lab-bounded-pilot-v1",
        )
    except (
        DevelopmentRunError,
        HoldoutRunError,
...
def _require_ready(capability: FrozenHoldoutCapability) -> None:
    ...
    try:
        ...
        refreshed = freeze_holdout_capability(
            capability.pilot_root,
            capability.freqtrade_python,
            capability.freqtrade_source,
            development_capability=(
                capability.development if capability.separate_root else None
            ),
        )
        if (
            refreshed.status != "READY"
            or refreshed.root_identity != capability.root_identity
            or refreshed.plan_sha256 != capability.plan_sha256
            or refreshed.acquisition_provenance_sha256
            != capability.acquisition_provenance_sha256
            or refreshed.config_sha256 != capability.config_sha256
            or refreshed.runner_sha256 != capability.runner_sha256
            or refreshed.development_timerange != capability.development_timerange
            or refreshed.holdout_timerange != capability.holdout_timerange
            or refreshed.stress_fee_multiplier != capability.stress_fee_multiplier
            or refreshed.pair != capability.pair
            or refreshed.local_receipts != capability.local_receipts
            or refreshed.separate_root != capability.separate_root
            or refreshed.root_schema != capability.root_schema
            or refreshed.bound_research_run_id
            != capability.bound_research_run_id
            or refreshed.bound_candidate_id != capability.bound_candidate_id
            or refreshed.bound_generation_run_id
            != capability.bound_generation_run_id
            or refreshed.bound_candidate_sha256
            != capability.bound_candidate_sha256
            or refreshed.bound_profile_id != capability.bound_profile_id
            or refreshed.bound_profile_sha256 != capability.bound_profile_sha256
            or refreshed.development_root_sha256
            != capability.development_root_sha256
            or refreshed.development_input_manifest_sha256
            != capability.development_input_manifest_sha256
            or refreshed.development_artifact_receipts
            != capability.development_artifact_receipts
        ):
            raise HoldoutRunError(
                "BLOCKED_DATA", "startup-frozen Holdout envelope changed"
            )
        pilot = capability.pilot_root
        acquisition = (
            pilot if capability.root_schema == HOLDOUT_ROOT_SCHEMA else pilot / "acquisition"
        )
        if capability.root_schema != HOLDOUT_ROOT_SCHEMA:
            fixed = (
...
def _materialize_holdout_inputs(
    ...
    acquisition = (
        capability.pilot_root
        if capability.root_schema == HOLDOUT_ROOT_SCHEMA
        else capability.pilot_root / "acquisition"
    )
        ...
        manifest = {
            ...
            "development_input_manifest_sha256": (
                ...
                if capability.root_schema == HOLDOUT_ROOT_SCHEMA
                    ...
                    _read_regular(
                        run_dir / "development-input" / "manifest.json",
            ...
            "development_root": {
                "schema": capability.development.root_schema,
                "manifest_sha256": capability.development.plan_sha256,
                "data_provenance_sha256": (
                    capability.development.source_provenance_sha256
                ),
            },
            "holdout_root": {
                "mode": "SEPARATE" if capability.separate_root else "LEGACY_COMBINED",
                "spec_sha256": capability.plan_sha256,
...
def _frozen_future_holdout(
    snapshot: Mapping[str, Any],
    checks: Mapping[str, Any],
    holdout_days: int,
) -> Optional[dict[str, Any]]:
    """Validate the post-Development future window without opening its data."""
    if snapshot.get("development_root_schema") != DEVELOPMENT_ROOT_SCHEMA:
        if checks != _ORIGINAL_CHECKS or "future_holdout" in snapshot:
            raise HoldoutRunError(
                "run_not_eligible", "Development continuation checks are invalid"
            )
        return None
    ...
    if (
        ...
        or checks != {**_ORIGINAL_CHECKS, "future_holdout": future}
        or future.get("policy") != "NEXT_UTC_DAY_AFTER_DEVELOPMENT_PASS_V1"
        or future.get("origin") != "CODEX_PROPOSED_USER_AUTHORIZED_DECOUPLING"
        or future.get("market_values") != "SEALED_UNREAD"
        or isinstance(holdout_days, bool)
        or not isinstance(holdout_days, int)
        or holdout_days <= 0
    ):
        raise HoldoutRunError(
            "run_not_eligible", "future Holdout contract is invalid"
        )
    start, stop = _timerange(future.get("timerange"))
    try:
        frozen_at = datetime.fromisoformat(
            str(future.get("frozen_at_utc")).replace("Z", "+00:00")
        )
    except ValueError as exc:
        raise HoldoutRunError(
            "run_not_eligible", "future Holdout contract is invalid"
        ) from exc
    expected_start = (frozen_at + timedelta(days=1)).replace(
        hour=0, minute=0, second=0, microsecond=0
    )
    if (
        frozen_at.utcoffset() != timedelta(0)
        or stop - start != timedelta(days=holdout_days)
        or start != expected_start
        or future.get("start_utc")
        != start.isoformat().replace("+00:00", "Z")
        or future.get("end_exclusive_utc")
...
def _authorization_receipt(
    ...
    *,
    ...
    current_development_input_manifest_sha256: str,
    current_development_artifact: Mapping[str, Any],
) -> dict[str, Any]:
    legacy_required = {
        "schema",
        "action",
    ...
    split_required = {
        ...
        "profile_binding_sha256",
        "development_root_manifest_sha256",
        "development_data_provenance_sha256",
        "holdout_root_manifest_sha256",
        "holdout_data_provenance_sha256",
        "development_input_manifest_sha256",
        "development_artifact",
        "freqtrade_source_tree",
        "freqtrade_python_identity",
    ...
    if not isinstance(raw, dict) or (
        raw.get("schema") == HOLDOUT_AUTHORIZATION_SCHEMA_V2
        and set(raw) != split_required
    ) or (
        raw.get("schema") != HOLDOUT_AUTHORIZATION_SCHEMA_V2
    ...
    split = raw["schema"] == HOLDOUT_AUTHORIZATION_SCHEMA_V2
    snapshot_split = (
        snapshot.get("development_root_schema") == DEVELOPMENT_ROOT_SCHEMA
    )
    if split != snapshot_split:
        raise HoldoutRunError(
            "BLOCKED_DATA", "Holdout authorization receipt schema drifted"
        )
    hashes.extend(
        (
            "development_root_manifest_sha256",
        ...
        if split
        else ("pilot_spec_sha256", "data_provenance_sha256")
    )
    for name in hashes:
        if not isinstance(raw[name], str) or _SHA256.fullmatch(raw[name]) is None:
            raise HoldoutRunError("BLOCKED_DATA", "Holdout authorization receipt is invalid")
    development_artifact = raw.get("development_artifact")
    if split and (
        not isinstance(development_artifact, dict)
        or set(development_artifact)
        != {"archive_sha256", "metadata_sha256", "provenance_sha256"}
        or any(
            not isinstance(value, str) or _SHA256.fullmatch(value) is None
            for value in development_artifact.values()
        )
    ):
        raise HoldoutRunError("BLOCKED_DATA", "Holdout authorization receipt is invalid")
    if (
    ...
    common_expected = (
        ...
        and raw["input_manifest_sha256"] == input_manifest_sha256
    ...
    if split:
        development_root = input_manifest.get("development_root")
        holdout_root = input_manifest.get("holdout_root")
        expected = (
            common_expected
            and raw["research_run_id"] == research_run_id
            and raw["candidate_id"] == candidate_id
            and raw["holdout_timerange"]
            == snapshot.get("future_holdout", {}).get("timerange")
            and isinstance(development_root, dict)
            and isinstance(holdout_root, dict)
            and holdout_root.get("mode") == "SEPARATE"
            and raw["development_root_manifest_sha256"]
            == snapshot.get("development_root_manifest_sha256")
            == development_root.get("manifest_sha256")
            and raw["development_data_provenance_sha256"]
            == snapshot.get("source_provenance_sha256")
            == development_root.get("data_provenance_sha256")
            and raw["holdout_root_manifest_sha256"]
            == holdout_root.get("spec_sha256")
            and raw["holdout_data_provenance_sha256"]
            == holdout_root.get("data_provenance_sha256")
            and raw["development_input_manifest_sha256"]
            == input_manifest.get("development_input_manifest_sha256")
            == current_development_input_manifest_sha256
            and development_artifact == input_manifest.get("development_artifact")
            == current_development_artifact
        )
    else:
        holdout_root = input_manifest.get("holdout_root")
        expected = (
            common_expected
            and raw["schema"] == HOLDOUT_AUTHORIZATION_SCHEMA
            and isinstance(holdout_root, dict)
            and holdout_root.get("mode") == "LEGACY_COMBINED"
            and raw["pilot_spec_sha256"] == snapshot.get("pilot_spec_sha256")
            and raw["data_provenance_sha256"]
            == snapshot.get("source_provenance_sha256")
            and start.isoformat().replace("+00:00", "Z")
            == snapshot.get("exclusive_stop_utc")
        )
    if not isinstance(raw["authorized_at"], str):
        raise HoldoutRunError(
            "BLOCKED_DATA", "Holdout authorization receipt is invalid"
        )
    authorized_at = _timestamp(raw["authorized_at"])
    if split:
        expected = expected and datetime.fromisoformat(
...
def _eligible_row(
    ...
    *,
    ...
    _frozen_future_holdout(snapshot, checks, int(row["holdout_days"]))
    if (
        ...
        or (
            snapshot.get("development_root_schema") == DEVELOPMENT_ROOT_SCHEMA
            and (
                snapshot.get("development_root_research_run_id")
                != research_run_id
                or not isinstance(
                    snapshot.get("development_root_manifest_sha256"), str
                )
                or _SHA256.fullmatch(
                    str(snapshot.get("development_root_manifest_sha256"))
                )
                is None
                or not isinstance(
                    snapshot.get("development_root_profile_binding_sha256"), str
                )
                or _SHA256.fullmatch(
                    str(snapshot.get("development_root_profile_binding_sha256"))
                )
                is None
            )
        )
...
def prepare_holdout_continuation(
    ...
    *,
    ...
    with closing(get_connection(database_path, must_exist=True)) as connection:
        ...
        development_capability = capability.development
        split_root = capability.root_schema == HOLDOUT_ROOT_SCHEMA
        future_holdout = snapshot.get("future_holdout") if split_root else None
        development_input_manifest_sha256 = hashlib.sha256(
            _read_regular(
                directory / "development-input" / "manifest.json",
                "Development input manifest",
                2 * 1024 * 1024,
            )
        ).hexdigest()
        artifact_receipts = {
            "archive_sha256": parsed.archive_sha256 if parsed is not None else None,
            "metadata_sha256": parsed.metadata_sha256 if parsed is not None else None,
            "provenance_sha256": parsed.provenance_sha256 if parsed is not None else None,
        }
        split_binding_mismatch = split_root and (
            not isinstance(future_holdout, dict)
            or capability.holdout_timerange != future_holdout.get("timerange")
            ...
            or capability.development_root_sha256
            != snapshot.get("development_root_manifest_sha256")
            or capability.development_input_manifest_sha256
            != development_input_manifest_sha256
            or dict(capability.development_artifact_receipts) != artifact_receipts
            or snapshot.get("development_root_profile_binding_sha256")
            != profile_binding_sha256
        )
        if (
            snapshot.get("pilot_spec_sha256")
            != development_capability.plan_sha256
            or snapshot.get("source_provenance_sha256")
            != development_capability.source_provenance_sha256
            or snapshot.get("config_sha256") != development_capability.config_sha256
            or snapshot.get("runner_sha256") != development_capability.runner_sha256
            or capability.config_sha256 != development_capability.config_sha256
            or capability.runner_sha256 != development_capability.runner_sha256
            or (
                development_capability.root_schema
                == DEVELOPMENT_ROOT_SCHEMA
                and snapshot.get("development_root_manifest_sha256")
                != development_capability.plan_sha256
            )
            or snapshot.get("timerange") != capability.development_timerange
            or snapshot.get("exclusive_stop_utc") != exclusive_stop_utc
            or (not split_root and development_stop != holdout_start)
            or split_binding_mismatch
            or holdout_duration <= timedelta(0)
            or (
    ...
    receipt: dict[str, Any] = {
        "schema": (
            HOLDOUT_AUTHORIZATION_SCHEMA_V2
            if capability.root_schema == HOLDOUT_ROOT_SCHEMA
            else HOLDOUT_AUTHORIZATION_SCHEMA
        ),
        "action": "AUTHORIZE_HOLDOUT",
        "authorized_at": timestamp,
        ...
        receipt.update(
            {
                ...
                "holdout_root_manifest_sha256": capability.plan_sha256,
                "holdout_data_provenance_sha256": (
                    capability.acquisition_provenance_sha256
                ),
                "development_input_manifest_sha256": (
    ...
    *,
    ...
    with closing(get_connection(database_path, must_exist=True)) as connection:
        try:
            ...
            _schema_v1(connection)
            row, current_development, snapshot = _eligible_row(
                connection, research_run_id, parse_artifact=True
            )
            if current_development is None:
                raise HoldoutRunError(
                    "run_state_conflict", "Development Artifact is unavailable"
                )
            persisted_manifest, persisted_receipt = _load_holdout_input(
                Path(str(row["run_dir"])), research_run_id
            )
            manifest_sha256 = hashlib.sha256(
                _read_regular(
                    Path(str(row["run_dir"])) / "holdout-input" / "manifest.json",
                    "Holdout manifest",
                    2 * 1024 * 1024,
                )
            ).hexdigest()
            current_development_input_manifest_sha256 = hashlib.sha256(
                _read_regular(
            ...
            current_development_artifact = {
                "archive_sha256": current_development.archive_sha256,
                "metadata_sha256": current_development.metadata_sha256,
                "provenance_sha256": current_development.provenance_sha256,
            }
            if dict(persisted_receipt) != dict(receipt):
                raise HoldoutRunError(
                    "BLOCKED_DATA", "Holdout authorization receipt drifted"
                )
            ...
            authorization = _authorization_receipt(
                ...
                input_manifest=persisted_manifest,
                input_manifest_sha256=manifest_sha256,
                current_development_input_manifest_sha256=(
                    current_development_input_manifest_sha256
                ),
                current_development_artifact=current_development_artifact,
            )
