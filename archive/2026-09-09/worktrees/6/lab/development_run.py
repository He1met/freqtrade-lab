...
DEVELOPMENT_ROOT_SCHEMA = "freqtrade-lab-development-root-v1"
DEVELOPMENT_ROOT_MANIFEST = "development-root.json"
FUTURE_HOLDOUT_POLICY = "NEXT_UTC_DAY_AFTER_DEVELOPMENT_PASS_V1"
EXPECTED_GATE = {
    "minimum_trades": 30,
...
class FrozenDevelopmentCapability:
    ...
    def public(self) -> dict[str, Any]:
        return {
            ...
            "data_root_schema": self.root_schema,
            "bound_candidate_id": self.bound_candidate_id,
            "bound_research_run_id": self.bound_research_run_id,
...
def _freeze_staged_development_root(
    root: Path,
    python: Path,
    source: Path,
) -> FrozenDevelopmentCapability:
    """Verify one immutable Development-only root with no later-phase data."""
    manifest_bytes = _read_bytes(
        root / DEVELOPMENT_ROOT_MANIFEST,
        "Development root manifest",
        2 * 1024 * 1024,
    )
    manifest = _json_bytes(manifest_bytes, "Development root manifest")
    required = {
        "schema",
    ...
    if set(manifest) != required or manifest.get("schema") != DEVELOPMENT_ROOT_SCHEMA:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root manifest is invalid")
    development_start, development_stop, _ = _development_window(
        manifest.get("development_timerange")
    )
    if manifest.get("gate") != {
        "version": DEVELOPMENT_GATE_VERSION,
        **EXPECTED_GATE,
    }:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root gate contract mismatch")
    ...
    if not isinstance(binding, dict) or set(binding) != binding_keys:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root binding is invalid")
    for name in (
        "research_run_id",
        "candidate_id",
        "generation_run_id",
        ...
        if (
            ...
            raise DevelopmentRunError("BLOCKED_DATA", "Development root binding is invalid")
    for name in ("candidate_code_sha256", "profile_binding_sha256"):
    ...
    if freqtrade != {
        ...
        raise DevelopmentRunError("BLOCKED_DATA", "Development root runtime contract mismatch")
    ...
    receipt_keys = (
        frozenset(source_receipts) if isinstance(source_receipts, dict) else None
    )
    if receipt_keys not in {
        frozenset(legacy_receipt_fields),
        frozenset(direct_receipt_fields),
    }:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root source receipt is invalid")
    assert isinstance(source_receipts, dict)
    direct_source = receipt_keys == frozenset(direct_receipt_fields)
    hash_fields = (
        ("source_provenance_sha256", "retrieval_receipt_sha256")
        if direct_source
        else tuple(legacy_receipt_fields)
    )
    ...
    if direct_source:
        try:
            ...
            for name in (
                ...
                parsed_receipt_times.append(parsed.astimezone(timezone.utc))
            source_start, source_stop, retrieval_finished = parsed_receipt_times
        except (KeyError, TypeError, ValueError) as exc:
            raise DevelopmentRunError(
                "BLOCKED_DATA", "Development root source receipt is invalid"
            ) from exc
        if (
            source_receipts.get("fully_closed_at_fetch") is not True
            or source_start != development_start - timedelta(hours=2)
            or source_stop != development_stop
            or retrieval_finished < development_stop
        ):
            raise DevelopmentRunError(
                "BLOCKED_DATA", "Development root source is not exactly closed"
            )

    files = manifest.get("files")
    if not isinstance(files, dict) or not files:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root file receipt is invalid")
    expected_roles = {
        "config.json": "config",
    ...
    config_record: Optional[Tuple[int, str]] = None
    market_record: Optional[Tuple[int, str]] = None
    tiers_record: Optional[Tuple[int, str]] = None
    for name, raw_record in files.items():
        if not isinstance(name, str):
            raise DevelopmentRunError("BLOCKED_DATA", "Development root path is invalid")
        relative = _safe_relative(name, "Development root input")
        if any(token in name.lower() for token in ("holdout", "stress")):
            raise DevelopmentRunError("BLOCKED_DATA", "Development root contains later-phase input")
        if not isinstance(raw_record, dict) or set(raw_record) != {"bytes", "sha256", "role"}:
            raise DevelopmentRunError("BLOCKED_DATA", "Development root file receipt is invalid")
        record = _receipt(raw_record, "Development root input")
        role = raw_record.get("role")
        expected_role = expected_roles.get(name)
        if expected_role is not None:
            if role != expected_role:
                raise DevelopmentRunError("BLOCKED_DATA", "Development root file role is invalid")
        elif name.startswith("data/okx/"):
            if role != "market_data":
                raise DevelopmentRunError("BLOCKED_DATA", "Development root file role is invalid")
        else:
            raise DevelopmentRunError("BLOCKED_DATA", "Development root has an extra input")
        _check_receipt(root / relative, record, "Development root input")
        seen_paths.add(name)
        if name == "config.json":
            config_record = record
        elif name == "market_snapshot.json":
            market_record = record
        elif name == "isolated_tiers_snapshot.json":
            tiers_record = record
        else:
            data_records.append((name.removeprefix("data/okx/"), record[0], record[1]))

    try:
        actual_paths = {
            path.relative_to(root).as_posix()
            for path in root.rglob("*")
            if path.is_file() and path.name != DEVELOPMENT_ROOT_MANIFEST
        }
        if any(path.is_symlink() for path in root.rglob("*")):
            raise OSError("symlink")
    except (OSError, RuntimeError, ValueError) as exc:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root layout is unsafe") from exc
    if actual_paths != seen_paths or not data_records:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root file set is invalid")
    if config_record is None or market_record is None or tiers_record is None:
        raise DevelopmentRunError("BLOCKED_DATA", "Development root input set is incomplete")

    config_data = _check_receipt(root / "config.json", config_record, "Development root config")
    config = _json_bytes(config_data, "Development root config")
    ...
    try:
        ...
        raise DevelopmentRunError("BLOCKED_DATA", "Development root config mismatch") from exc
    source_value = manifest.get("source")
    market_bytes = _check_receipt(
        root / "market_snapshot.json", market_record, "Development root market snapshot"
    )
    pair, instrument_id = _verified_market_identity(
        source_value,
        market_bytes,
        source_value.get("pair") if isinstance(source_value, dict) else None,
    )
    if (
        not math.isclose(validated_fee, 0.0005, abs_tol=1e-15)
        or validated_pairs != (pair,)
        ...
        raise DevelopmentRunError("BLOCKED_DATA", "Development root config mismatch")
    ...
    development_provenance_sha256 = (
        _sha256(
            _canonical_bytes(
                {
                    "schema": "freqtrade-lab-derived-development-provenance-v1",
                    "development_timerange": manifest["development_timerange"],
        ...
        if direct_source
        else str(source_receipts["development_provenance_sha256"])
    )
    return FrozenDevelopmentCapability(
        status="READY",
        reason="immutable Development-only Freqtrade 2026.7 capability is frozen",
        pilot_root=root,
        freqtrade_python=python,
        freqtrade_source=source,
        pilot_identity=(root_info.st_dev, root_info.st_ino),
        python_identity=_python_identity(python),
        source_identity=(source_info.st_dev, source_info.st_ino),
        plan_sha256=_sha256(manifest_bytes),
        source_provenance_sha256=str(source_receipts["source_provenance_sha256"]),
        ...
        window_schema=DEVELOPMENT_ROOT_SCHEMA,
        development_timerange=str(manifest["development_timerange"]),
        pair=pair,
...
def freeze_development_capability(
    ...
    try:
        ...
        manifest = pilot / DEVELOPMENT_ROOT_MANIFEST
        if manifest.exists() or manifest.is_symlink():
            return _freeze_staged_development_root(pilot, python, source)
...
def prepare_development_root(
    ...
    *,
    ...
    """Publish one immutable Development-only root from a verified legacy Pilot.

    The published root contains only the physical Development view plus the
    config and public market/leverage snapshots required by the existing
    runner.  No Holdout or Stress market file is copied.
    """
    ...
    if (
        ...
        or capability.pair is None
        or capability.instrument_id is None
        or capability.plan_sha256 is None
        or capability.source_provenance_sha256 is None
        or capability.development_provenance_sha256 is None
        or capability.config_sha256 is None
        or capability.market_receipt is None
        or capability.tiers_receipt is None
    ):
        raise DevelopmentRunError(
            "BLOCKED_DATA", "verified legacy Development source is unavailable"
        )
    ...
    try:
        ...
        raise DevelopmentRunError(
            "BLOCKED_DATA", "research_run_id must be a canonical UUIDv4"
        ) from exc
    ...
    with closing(connection):
        ...
        _profile_gate(candidate, capability)
    ...
    try:
        capability.pilot_root.relative_to(project_root)
    except ValueError:
        pass
    else:
        raise DevelopmentRunError(
            "BLOCKED_DATA", "Development source root must stay outside Git"
        )
    ...
    try:
        ...
        try:
            ...
            raise OSError("target is inside Git")
    except (OSError, RuntimeError, ValueError) as exc:
        raise DevelopmentRunError(
            "BLOCKED_DATA", "Development output root must be a new path"
        ) from exc
    staging = parent / f".{target.name}.preparing-{uuid4()}"
    acquisition = capability.pilot_root / "acquisition"
    isolation = capability.pilot_root / "development-isolation"
    if (trusted_source_provenance_sha256 is None) != (
        trusted_source_receipt_sha256 is None
    ):
        raise DevelopmentRunError(
            "BLOCKED_DATA", "both trusted Development source hashes are required"
        )
    if trusted_source_provenance_sha256 is not None:
        if (
            _SHA256.fullmatch(trusted_source_provenance_sha256) is None
            or trusted_source_receipt_sha256 is None
            or _SHA256.fullmatch(trusted_source_receipt_sha256) is None
        ):
            raise DevelopmentRunError(
                "BLOCKED_DATA", "trusted Development source hashes are invalid"
            )
        source_provenance_bytes = _read_bytes(
            acquisition / "retained-data-provenance.json",
            "source provenance",
        )
        source_provenance = _json_bytes(
            source_provenance_bytes, "source provenance"
        )
        source_identity = source_provenance.get("source")
        receipt_name = (
            source_identity.get("retrieval_receipt")
            if isinstance(source_identity, dict)
            else None
        )
        receipt_path = acquisition / _safe_relative(
            receipt_name, "source retrieval receipt"
        )
        receipt_bytes = _read_bytes(receipt_path, "source retrieval receipt")
        if (
            _sha256(source_provenance_bytes)
            != trusted_source_provenance_sha256
            or _sha256(receipt_bytes) != trusted_source_receipt_sha256
        ):
            raise DevelopmentRunError(
                "BLOCKED_DATA", "trusted Development source hashes do not match"
            )
    published = False
    try:
        ...
        def copy(relative: str, source_path: Path, record: Tuple[int, str], role: str) -> None:
            data = _check_receipt(source_path, record, "Development source input")
            destination = staging / _safe_relative(relative, "Development output input")
        ...
        config_data = _read_bytes(acquisition / "config.json", "Pilot config")
        if _sha256(config_data) != capability.config_sha256:
            raise DevelopmentRunError("BLOCKED_DATA", "Pilot config changed")
        copy(
            "config.json",
            acquisition / "config.json",
            (len(config_data), capability.config_sha256),
            "config",
        )
        copy(
        ...
        manifest = {
            ...
            "source_receipts": {
                "source_contract_sha256": capability.plan_sha256,
        ...
        _write_exclusive(
            staging / DEVELOPMENT_ROOT_MANIFEST,
            manifest_bytes,
            0o400,
        )
        ...
        staging.chmod(0o500)
        try:
            _publish_directory_exclusive(staging, target)
        except ResearchCandidateError as exc:
            raise DevelopmentRunError(
                "BLOCKED_DATA", "Development output root was claimed concurrently"
            ) from exc
        published = True
        ...
        raise DevelopmentRunError(
            "BLOCKED_DATA", "Development output root could not be published"
        ) from exc
    finally:
...
def _require_ready(capability: FrozenDevelopmentCapability) -> None:
    ...
    frozen = (
        ...
        capability.root_schema,
        capability.bound_research_run_id,
        capability.bound_candidate_id,
        capability.bound_generation_run_id,
        capability.bound_candidate_sha256,
        capability.bound_profile_id,
        capability.bound_profile_sha256,
    )
    current = (
        ...
        refreshed.root_schema,
        refreshed.bound_research_run_id,
        refreshed.bound_candidate_id,
        refreshed.bound_generation_run_id,
        refreshed.bound_candidate_sha256,
        refreshed.bound_profile_id,
        refreshed.bound_profile_sha256,
    )
...
def _profile_binding_sha256(
    ...
    if row is None:
        raise DevelopmentRunError("BLOCKED_PROFILE", "Profile is unavailable")
    return _sha256(_canonical_bytes({key: row[key] for key in row.keys()}))
...
def _profile_gate(row: sqlite3.Row, capability: FrozenDevelopmentCapability) -> None:
    ...
    expected = (
        ...
        and (
            (
                capability.root_schema != DEVELOPMENT_ROOT_SCHEMA
                and capability.window_schema != _STRICT_WINDOW_SCHEMA
            )
            or int(row["holdout_days"]) == 30
        )
...
def _snapshot(capability: FrozenDevelopmentCapability, row: sqlite3.Row) -> dict[str, Any]:
    ...
    return {
        ...
        "development_root_research_run_id": capability.bound_research_run_id,
        "development_root_profile_binding_sha256": capability.bound_profile_sha256,
        "security_gate_version": BOUNDED_CAUSAL_STRATEGY_V1,
        "pilot_spec_sha256": capability.plan_sha256,
        "development_root_schema": capability.root_schema,
        "development_root_manifest_sha256": (
            capability.plan_sha256
            if capability.root_schema == DEVELOPMENT_ROOT_SCHEMA
            else None
        ),
        "source_provenance_sha256": capability.source_provenance_sha256,
        "development_provenance_sha256": capability.development_provenance_sha256,
...
def _materialize_inputs(
    ...
    if capability.root_schema == DEVELOPMENT_ROOT_SCHEMA:
        acquisition = capability.pilot_root
        isolation = capability.pilot_root
    else:
        acquisition = capability.pilot_root / "acquisition"
        isolation = capability.pilot_root / "development-isolation"
...
def prepare_development_run(
    ...
    *,
    ...
    if (
        capability.bound_research_run_id is not None
        and research_run_id is not None
        and research_run_id != capability.bound_research_run_id
    ):
        raise DevelopmentRunError(
            "BLOCKED_DATA", "Development root research_run_id binding mismatch"
        )
    run_id = (
        capability.bound_research_run_id
        or research_run_id
    ...
    try:
        ...
        if capability.root_schema == DEVELOPMENT_ROOT_SCHEMA and (
            row["id"] != capability.bound_candidate_id
            or row["generation_run_id"] != capability.bound_generation_run_id
            or row["code_sha256"] != capability.bound_candidate_sha256
            or row["research_profile_id"] != capability.bound_profile_id
            or _profile_binding_sha256(connection, str(row["research_profile_id"]))
            != capability.bound_profile_sha256
        ):
            raise DevelopmentRunError(
                "BLOCKED_DATA", "Development root Candidate/Profile binding drifted"
            )
...
def finalize_development_gate(database_path: PathLike, research_run_id: str) -> dict[str, Any]:
    ...
    with closing(get_connection(database_path, must_exist=True)) as connection:
        try:
            ...
            if passed and snapshot.get("development_root_schema") == DEVELOPMENT_ROOT_SCHEMA:
                if snapshot.get(
                    "development_root_profile_binding_sha256"
                ) != _profile_binding_sha256(
                    connection, str(snapshot.get("research_profile_id"))
                ):
                    raise DevelopmentRunError(
                        "run_state_conflict",
                        "Profile changed before future Holdout was frozen",
                    )
                holdout_days_row = connection.execute(
                    "SELECT holdout_days FROM research_profiles WHERE id=?",
                    (snapshot.get("research_profile_id"),),
                ).fetchone()
                if (
                    holdout_days_row is None
                    or isinstance(holdout_days_row[0], bool)
                    or not isinstance(holdout_days_row[0], int)
                    or int(holdout_days_row[0]) != 30
                ):
                    raise DevelopmentRunError(
                        "run_state_conflict", "future Holdout duration is invalid"
                    )
                passed_at = datetime.fromisoformat(finished.replace("Z", "+00:00"))
                holdout_start = (passed_at + timedelta(days=1)).replace(
                    hour=0,
                    minute=0,
                    second=0,
                    microsecond=0,
                )
                holdout_stop = holdout_start + timedelta(days=int(holdout_days_row[0]))
                future_holdout = {
                    "policy": FUTURE_HOLDOUT_POLICY,
                    "origin": "CODEX_PROPOSED_USER_AUTHORIZED_DECOUPLING",
                    "timerange": (
                        f"{holdout_start:%Y%m%d}-{holdout_stop:%Y%m%d}"
                    ),
                    "start_utc": holdout_start.isoformat().replace("+00:00", "Z"),
                    "end_exclusive_utc": holdout_stop.isoformat().replace(
                        "+00:00", "Z"
                    ),
                    "frozen_at_utc": finished,
                    "market_values": "SEALED_UNREAD",
                }
                snapshot["future_holdout"] = future_holdout
                checks["future_holdout"] = future_holdout
            connection.execute(
                """
            ...
            connection.execute(
                ...
                SET status = ?, stage = ?, verdict = ?, input_snapshot_json = ?, checks_json = ?,
...
def _public_checks(raw: Any) -> dict[str, Any]:
    try:
    ...
    has_future = frozenset(value) == required | {"future_holdout"}
    if frozenset(value) != required and not has_future:
        raise DevelopmentRunError(
    ...
    for key, allowed in _PUBLIC_CHECK_VALUES.items():
        ...
        public[key] = selected
    if has_future:
        future = value.get("future_holdout")
        if (
            ...
            or future.get("policy") != FUTURE_HOLDOUT_POLICY
            or future.get("origin")
            != "CODEX_PROPOSED_USER_AUTHORIZED_DECOUPLING"
            or future.get("market_values") != "SEALED_UNREAD"
        ):
            raise DevelopmentRunError(
                "run_state_conflict", "future Holdout check is invalid"
            )
        try:
            parts = str(future.get("timerange")).split("-", 1)
        ...
        if (
            ...
            or start
            != (frozen_at + timedelta(days=1)).replace(
        ...
        public["future_holdout"] = dict(future)
    return public
...
def research_context(
    ...
    with closing(connection):
        ...
        staged = (
            capability.root_schema == DEVELOPMENT_ROOT_SCHEMA
            and isinstance(capability.bound_candidate_id, str)
            and isinstance(capability.bound_research_run_id, str)
        )
        if staged:
            latest_row = connection.execute(
                """
                SELECT id FROM research_runs
                WHERE id=? AND pipeline_version=?
                """,
                (
                    capability.bound_research_run_id,
                    DEVELOPMENT_PIPELINE_VERSION,
            ...
            ids = [str(capability.bound_candidate_id)]
        else:
            latest_row = connection.execute(
                """
                SELECT id FROM research_runs
