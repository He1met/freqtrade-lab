...
class ResearchConsoleConfig:
    ...
    holdout_root: Optional[Path]
    runtime_identity: Tuple[int, int]
    campaigns_identity: Tuple[int, int]
    pilot_identity: Optional[Tuple[int, int]]
    development_identity: Optional[Tuple[int, int]]
    holdout_identity: Optional[Tuple[int, int]]
    legacy_combined_root: bool
...
def build_check_development_argv(
    development_root: Path,
    python_executable: PathLike = sys.executable,
    freqtrade_python: Optional[PathLike] = None,
    freqtrade_source: Optional[PathLike] = None,
) -> Tuple[str, ...]:
    """Verify one staged Development root without touching Holdout market data."""
    script = PILOT_SCRIPT.resolve(strict=True)
    argv = [
        str(python_executable),
        str(script),
    ...
    if freqtrade_python is not None:
        argv.extend(("--freqtrade-python", str(freqtrade_python)))
    if freqtrade_source is not None:
        argv.extend(("--freqtrade-source", str(freqtrade_source)))
    return tuple(argv)
...
class ResearchConsoleController:
    ...
    def __init__(
        ...
        pilot_root: Optional[PathLike] = None,
        *,
        development_root: Optional[PathLike] = None,
        holdout_root: Optional[PathLike] = None,
        ...
        if pilot_root is not None and development_root is not None:
            raise ResearchConsoleError(
                "choose either pilot root legacy alias or development root"
            )
        selected_development_root = (
            development_root if development_root is not None else pilot_root
        )
        if selected_development_root is None:
            raise ResearchConsoleError("development root is required")
        development_label = (
            "development root" if development_root is not None else "pilot root"
        )
        development, development_identity = _resolve_pilot_directory(
            selected_development_root,
            development_label,
        )
        legacy_combined_root = development_root is None and holdout_root is None
        if legacy_combined_root:
            holdout = development
            holdout_identity = development_identity
        elif holdout_root is None:
            holdout = None
            holdout_identity = None
        else:
            holdout, holdout_identity = _resolve_pilot_directory(
                holdout_root,
                "holdout root",
            )
        ...
        try:
            ...
            self.config = ResearchConsoleConfig(
                ...
                pilot_identity=development_identity,
                development_identity=development_identity,
                holdout_identity=holdout_identity,
                legacy_combined_root=legacy_combined_root,
            ...
            self.check_data_argv = (
                build_check_data_argv(development, resolved_python)
                if legacy_combined_root
                else build_check_development_argv(
                    development,
                    resolved_python,
                    self.config.freqtrade_python,
                    self.config.freqtrade_source,
                )
            )
            ...
            if legacy_combined_root:
                ...
                self._holdout_capability = FrozenHoldoutCapability(
                    status="BLOCKED_DATA",
                    reason="Holdout root is not configured; market values remain sealed",
                    development=self._development_capability,
                    separate_root=True,
                )
            else:
                self._holdout_capability = freeze_holdout_capability(
                    ...
                    freqtrade_python,
                    freqtrade_source,
                    development_capability=self._development_capability,
                )
    ...
    def preflight(self) -> Dict[str, Any]:
        ...
        development_ok = self._directory_unchanged(
            self.config.development_root,
            self.config.development_identity,
        )
        holdout_ok = (
            self.config.holdout_root is not None
            and self._directory_unchanged(
                self.config.holdout_root,
                self.config.holdout_identity,
            )
        )
        ...
        checks = {
            ...
            "pilot_root": {
                "status": "READY" if development_ok else "UNAVAILABLE",
                "outside_git": development_ok,
                "legacy_alias": True,
                "message": (
                    "Git 外 Pilot 目录已冻结"
                    if development_ok
            ...
            "development_root": {
                ...
                "outside_git": development_ok,
                "message": (
                    "Git 外 Development 目录已冻结"
                    if development_ok
                    else "Development 目录身份已变化"
                ),
            },
            "holdout_root": {
                "status": "READY" if holdout_ok else "BLOCKED_DATA",
                "outside_git": holdout_ok,
                "configured": self.config.holdout_root is not None,
                "sealed": True,
                "message": (
                    "Git 外 Holdout 目录已冻结，市场值保持封存"
                    if holdout_ok
                    else "Holdout 目录未配置或身份不可用，市场值保持封存"
                ),
            },
    ...
    def _decorate_research_run(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            binding_matches = (
                not self._holdout_capability.separate_root
                or (
                    isinstance(run_id, str)
                    and self._holdout_capability.bound_research_run_id == run_id
                )
            )
        ...
        if database_allows and not controller_allows:
            if not capability_ready:
                ...
                normalized["reason"] = "ResearchRun 不属于当前冻结的 Holdout 根"
    ...
    def create_research_run(self, candidate_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            run_id = (
                self._development_capability.bound_research_run_id
                or str(uuid4())
            )
