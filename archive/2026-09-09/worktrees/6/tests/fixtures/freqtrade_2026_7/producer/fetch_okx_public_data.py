...
DEVELOPMENT_WINDOW_SPEC_SCHEMA = "freqtrade-lab-okx-development-window-v1"
...
def load_window_spec(
    ...
    if value.get("schema") == DEVELOPMENT_WINDOW_SPEC_SCHEMA:
        expected = {
            "schema",
            "data_start_utc",
            "development_start_utc",
        ...
        if missing:
            raise RuntimeError(
                f"window spec is missing fields: {', '.join(missing)}"
            )
        if unknown:
            raise RuntimeError(
                f"window spec contains unknown fields: {', '.join(unknown)}"
            )
        data_start = _utc_z_datetime(
            value["data_start_utc"], "window spec data_start_utc"
        )
        development_start = _utc_z_datetime(
            value["development_start_utc"],
            "window spec development_start_utc",
        )
        development_end = _utc_z_datetime(
            value["end_exclusive_utc"], "window spec end_exclusive_utc"
        )
        if development_end - development_start != timedelta(days=60):
            raise RuntimeError(
                "development window v1 requires exactly 60 Development days"
            )
        if not data_start < development_start < development_end:
            raise RuntimeError(
                "development window must satisfy data_start < development_start "
                "< end_exclusive"
            )
        if any(
            (
                development_start.hour,
                development_start.minute,
                development_start.second,
                development_start.microsecond,
                development_end.hour,
                development_end.minute,
                development_end.second,
                development_end.microsecond,
            )
        ):
            raise RuntimeError(
                "development window boundaries must be UTC midnight"
            )
        if any((data_start.minute, data_start.second, data_start.microsecond)):
            raise RuntimeError(
                "window spec data_start_utc must be aligned to an UTC hour"
            )
        if development_start - data_start != timedelta(hours=2):
            raise RuntimeError(
                "development window v1 requires exactly two startup hours"
            )
        if development_end >= datetime.now(UTC):
            raise RuntimeError(
                "window spec end_exclusive_utc must be fully closed"
            )
        return (
            DEVELOPMENT_WINDOW_SPEC_SCHEMA,
            data_start,
            development_start,
            development_end,
            development_end,
...
def acquire(root: Path, runtime: dict[str, object]) -> Path:
    ...
    elif WINDOW_SCHEMA_USED == DEVELOPMENT_WINDOW_SPEC_SCHEMA:
        receipt["data_window"] = {
            "schema": DEVELOPMENT_WINDOW_SPEC_SCHEMA,
            "start_utc": DATA_START.isoformat(),
            "development_start_utc": DEVELOPMENT_START.isoformat(),
            "end_exclusive_utc": DATA_END.isoformat(),
            "fully_closed_at_fetch": True,
...
def write_local_producer_inputs(
    ...
    if WINDOW_SCHEMA_USED != DEVELOPMENT_WINDOW_SPEC_SCHEMA:
        provenance["contract"]["holdout_timerange"] = scenario_timerange(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "optional window JSON; development-window-v1 freezes only one "
            "closed 60-day Development view, while window-v2 freezes the "
            "legacy 60-day Development plus 30-day Holdout acquisition"
        ),
    )
