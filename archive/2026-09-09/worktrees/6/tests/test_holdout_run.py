...
from tests.test_development_run import (
    ...
    _prepared_development_root_fixture,
...
def _pass_development_gate(
    ...
    *,
    ...
    """Attach TEST_ONLY_SYNTHETIC evidence and pass the real fixed Gate."""
    ...
    parsed = replace(
        ...
        strategy_sha256=_sha256(BOUNDED_SOURCE.encode("utf-8")),
        pairs=(pair,),
...
class _StagedSplitRun:
    database: Path
    candidate_id: str
    research_run_id: str
    run_dir: Path
    development_root: Path
    development: development_run.FrozenDevelopmentCapability
    holdout_root: Path
    holdout: holdout_run.FrozenHoldoutCapability
    parsed: backtest_artifact.ParsedBacktestArtifact
    snapshot: dict[str, Any]
    authorization_now: str


def _publish_staged_holdout_root(
    root: Path,
    development_root: Path,
    run_dir: Path,
    development: development_run.FrozenDevelopmentCapability,
    ...
    """Publish a TEST_ONLY_SYNTHETIC future root with exact source receipts."""
    holdout_start, holdout_stop = _timerange_datetimes(holdout_timerange)
    root.mkdir()
    ...
    for relative, role in roles.items():
        data = (development_root / relative).read_bytes()
    ...
    for source in sorted((development_root / "data" / "okx").rglob("*")):
        if not source.is_file():
            continue
        relative = source.relative_to(development_root).as_posix()
        data = source.read_bytes() + b"TEST_ONLY_SYNTHETIC future Holdout bytes\n"
    ...
    development_manifest = json.loads(
        (development_root / development_run.DEVELOPMENT_ROOT_MANIFEST).read_text()
    )
    development_input_manifest_sha256 = _sha256(
        (run_dir / "development-input" / "manifest.json").read_bytes()
    )
    artifact_receipts = {
    ...
    source_stop = holdout_stop
    source_receipt = {
        "pair": development.pair,
        "instrument_id": development.instrument_id,
        "source_start_utc": source_start.isoformat().replace("+00:00", "Z"),
        "source_end_exclusive_utc": source_stop.isoformat().replace("+00:00", "Z"),
        "retrieval_finished_at_utc": source_stop.isoformat().replace("+00:00", "Z"),
        "fully_closed_at_fetch": True,
    }
    manifest = {
        "schema": holdout_run.HOLDOUT_ROOT_SCHEMA,
        "binding": {
            "research_run_id": development.bound_research_run_id,
            "candidate_id": development.bound_candidate_id,
            "generation_run_id": development.bound_generation_run_id,
            "candidate_code_sha256": development.bound_candidate_sha256,
            "research_profile_id": development.bound_profile_id,
            "profile_binding_sha256": development.bound_profile_sha256,
            "development_root_manifest_sha256": development.plan_sha256,
            "development_input_manifest_sha256": development_input_manifest_sha256,
            "development_artifact": artifact_receipts,
        },
        "holdout_timerange": holdout_timerange,
        "stress_fee_multiplier": 2.0,
        "policy": {
            "future_window": development_run.FUTURE_HOLDOUT_POLICY,
            "max_open_count": 1,
        ...
        "freqtrade": development_manifest["freqtrade"],
        "source_receipts": {
            "source_provenance_sha256": _sha256(
                _canonical({"kind": "future-holdout-provenance", **source_receipt})
            ),
            "retrieval_receipt_sha256": _sha256(
                _canonical({"kind": "future-holdout-retrieval", **source_receipt})
            ),
            "retrieval_finished_at_utc": source_receipt[
                "retrieval_finished_at_utc"
            ],
            "source_start_utc": source_receipt["source_start_utc"],
            "source_end_exclusive_utc": source_receipt[
    ...
    (root / holdout_run.HOLDOUT_ROOT_MANIFEST).write_bytes(_canonical(manifest))


def _staged_split_run(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    *,
    publish_holdout: bool = True,
) -> _StagedSplitRun:
    (
        database,
        candidate_id,
        ...
        _source,
        development,
    ) = _prepared_development_root_fixture(tmp_path, monkeypatch)
    run_dir = tmp_path / "runtime" / research_run_id
    run_dir.mkdir(parents=True)
    development_run.prepare_development_run(
        database,
    ...
    assert development.pair is not None
    assert development.development_timerange is not None
    parsed = _pass_development_gate(
        database,
        research_run_id,
        run_dir,
    ...
    future = snapshot["future_holdout"]
    _, holdout_stop = _timerange_datetimes(future["timerange"])
    authorization_now = holdout_stop.isoformat().replace("+00:00", "Z")
    holdout_root = tmp_path / "future-holdout-root"
    if publish_holdout:
        _publish_staged_holdout_root(
            holdout_root,
            development_root,
            run_dir,
            development,
    ...
    holdout = holdout_run.freeze_holdout_capability(
        holdout_root,
        development.freqtrade_python,
        development.freqtrade_source,
        development_capability=development,
    )
    return _StagedSplitRun(
        database=database,
...
def _prepare_split_continuation(
    staged: _StagedSplitRun,
    monkeypatch: pytest.MonkeyPatch,
    capability: holdout_run.FrozenHoldoutCapability | None = None,
) -> holdout_run.PreparedHoldoutContinuation:
    monkeypatch.setattr(
        holdout_run,
        "parse_backtest_artifact",
        lambda *_args, **_kwargs: staged.parsed,
    )
    return holdout_run.prepare_holdout_continuation(
        staged.database,
        staged.run_dir,
        staged.research_run_id,
        capability or staged.holdout,
        now=staged.authorization_now,
    )
...
def test_t0_missing_separate_holdout_root_stays_sealed_and_blocked(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    staged = _staged_split_run(tmp_path, monkeypatch, publish_holdout=False)

    assert staged.development.status == "READY"
    assert staged.snapshot["holdout"] == "SEALED_UNREAD"
    assert staged.snapshot["holdout_stress"] == "SEALED_UNREAD"
    assert not staged.holdout_root.exists()
    assert staged.holdout.status == "BLOCKED_DATA"
    assert staged.holdout.public()["holdout"] == "SEALED_UNREAD"
    assert staged.holdout.public()["holdout_stress"] == "SEALED_UNREAD"

    with pytest.raises(holdout_run.HoldoutRunError) as raised:
        _prepare_split_continuation(staged, monkeypatch)

    assert raised.value.code == "BLOCKED_DATA"
    _assert_no_later_rows(staged.database, staged.research_run_id)
    assert not (staged.run_dir / holdout_run.HOLDOUT_ATTEMPT_NAME).exists()
    assert not (staged.run_dir / "holdout-input").exists()


def test_t1_split_roots_v2_authorization_cross_binds_all_evidence(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    staged = _staged_split_run(tmp_path, monkeypatch)
    development_start, development_stop = _timerange_datetimes(
        staged.development.development_timerange
    )
    holdout_start, holdout_stop = _timerange_datetimes(
        staged.holdout.holdout_timerange
    )

    assert staged.holdout.status == "READY", staged.holdout.reason
    assert staged.development_root != staged.holdout_root
    assert development_start < development_stop < holdout_start < holdout_stop
    assert staged.holdout.acquisition_provenance_sha256 != (
        staged.development.source_provenance_sha256
    )
    prepared = _prepare_split_continuation(staged, monkeypatch)

    assert prepared.research_run_id == staged.research_run_id
    input_root = staged.run_dir / "holdout-input"
    authorization_bytes = (input_root / "authorization.json").read_bytes()
    authorization = json.loads(authorization_bytes)
    input_manifest_bytes = (input_root / "manifest.json").read_bytes()
    input_manifest = json.loads(input_manifest_bytes)
    development_input_manifest_sha256 = _sha256(
        (staged.run_dir / "development-input" / "manifest.json").read_bytes()
    )
    development_artifact = {
        "archive_sha256": staged.parsed.archive_sha256,
        "metadata_sha256": staged.parsed.metadata_sha256,
        "provenance_sha256": staged.parsed.provenance_sha256,
    }
    expected_binding = {
        "schema": holdout_run.HOLDOUT_AUTHORIZATION_SCHEMA_V2,
        "research_run_id": staged.research_run_id,
        "candidate_id": staged.candidate_id,
        "candidate_code_sha256": staged.development.bound_candidate_sha256,
        "research_profile_id": staged.development.bound_profile_id,
        "profile_binding_sha256": staged.development.bound_profile_sha256,
        "development_root_manifest_sha256": staged.development.plan_sha256,
        "development_data_provenance_sha256": (
            staged.development.source_provenance_sha256
        ),
        "holdout_root_manifest_sha256": staged.holdout.plan_sha256,
        "holdout_data_provenance_sha256": (
            staged.holdout.acquisition_provenance_sha256
        ),
        "development_input_manifest_sha256": development_input_manifest_sha256,
        "development_artifact": development_artifact,
    }
    assert {name: authorization[name] for name in expected_binding} == expected_binding
    assert authorization["input_manifest_sha256"] == _sha256(input_manifest_bytes)
    assert authorization["holdout_timerange"] == staged.snapshot["future_holdout"][
        "timerange"
    ]
    assert input_manifest["development_input_manifest_sha256"] == (
        development_input_manifest_sha256
    )
    assert input_manifest["development_root"] == {
        "schema": development_run.DEVELOPMENT_ROOT_SCHEMA,
        "manifest_sha256": staged.development.plan_sha256,
        "data_provenance_sha256": staged.development.source_provenance_sha256,
    }
    assert input_manifest["holdout_root"] == {
        "mode": "SEPARATE",
        "spec_sha256": staged.holdout.plan_sha256,
        "data_provenance_sha256": staged.holdout.acquisition_provenance_sha256,
    }
    assert input_manifest["development_artifact"] == development_artifact
    source_receipts = json.loads(
        (staged.holdout_root / holdout_run.HOLDOUT_ROOT_MANIFEST).read_text()
    )["source_receipts"]
    source_start = source_receipts["source_start_utc"]
    source_stop = source_receipts["source_end_exclusive_utc"]
    retrieved = source_receipts["retrieval_finished_at_utc"]
    assert source_start == (holdout_start - timedelta(hours=2)).isoformat().replace(
        "+00:00", "Z"
    )
    assert source_stop == holdout_stop.isoformat().replace("+00:00", "Z")
    assert retrieved == source_stop
    assert source_receipts["fully_closed_at_fetch"] is True
    assert [
        (row["scenario"], row["sequence"], row["status"])
        for row in _execution_rows(staged.database, staged.research_run_id)
    ] == [
...
@pytest.mark.parametrize(
    ...
    (
        ...
        "holdout_data_provenance_sha256",
        "development_input_manifest_sha256",
        "development_artifact.archive_sha256",
    ),
)
def test_t0_split_v2_authorization_binding_drift_fails_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    binding: str,
) -> None:
    ...
    if binding == "schema":
        forged[binding] = holdout_run.HOLDOUT_AUTHORIZATION_SCHEMA
    elif binding == "authorized_at":
        forged[binding] = staged.snapshot["future_holdout"]["start_utc"]
    elif binding == "holdout_timerange":
        forged[binding] = "20300101-20300131"
    elif "." in binding:
        parent, child = binding.split(".", 1)
        forged[parent][child] = "0" * 64
    elif binding.endswith("sha256"):
        forged[binding] = "0" * 64
    else:
        forged[binding] = f"drifted-{binding}"
    authorization_path = staged.run_dir / "holdout-input" / "authorization.json"
    authorization_path.chmod(0o600)
    authorization_path.write_bytes(_canonical(forged))
    ...
    assert raised.value.code == "BLOCKED_DATA"
    _assert_no_later_rows(staged.database, staged.research_run_id)
    assert (staged.run_dir / holdout_run.HOLDOUT_ATTEMPT_NAME).is_file()
