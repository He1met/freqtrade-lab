...
def _prepared_development_root_fixture(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> tuple[Path, str, str, Path, Path, development_run.FrozenDevelopmentCapability]:
    """Publish and freeze one Candidate-bound Development-only root."""
    ...
    research_run_id = str(uuid4())
    output_parent = tmp_path / "published-roots"
    output_parent.mkdir()
    development_root = output_parent / "development"

    published = development_run.prepare_development_root(
        pilot,
        development_root,
        python,
        source,
    ...
    assert published["research_run_id"] == research_run_id
...
def test_t0_prepared_development_root_is_bound_and_contains_no_holdout(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (
    ...
    manifest_path = development_root / development_run.DEVELOPMENT_ROOT_MANIFEST
    manifest_bytes = manifest_path.read_bytes()
    manifest = json.loads(manifest_bytes)
    with get_connection(database, read_only=True) as connection:
        candidate = connection.execute(
            """
            SELECT c.generation_run_id, c.code_sha256, gr.research_profile_id
        ...
        profile_sha256 = development_run._profile_binding_sha256(
            connection,
    ...
    assert manifest["binding"] == {
        ...
        "research_profile_id": candidate["research_profile_id"],
        "profile_binding_sha256": profile_sha256,
    }
    assert capability.plan_sha256 == hashlib.sha256(manifest_bytes).hexdigest()
    assert capability.bound_research_run_id == research_run_id
    assert capability.bound_candidate_id == candidate_id
    assert capability.bound_profile_sha256 == profile_sha256
    relative_files = {
        path.relative_to(development_root).as_posix()
        for path in development_root.rglob("*")
        if path.is_file()
    }
    assert development_run.DEVELOPMENT_ROOT_MANIFEST in relative_files
    assert any(name.startswith("data/okx/") for name in relative_files)
    assert not any(
        token in name.lower()
        for name in relative_files
        for token in ("holdout", "stress")
    )
    assert not (tmp_path / "future-holdout-root").exists()
    public = capability.public()
...
def test_t0_direct_development_receipt_is_transparent_and_accepts_fractional_finish(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (
    ...
    manifest["source_receipts"] = {
        ...
        "source_start_utc": "2026-03-31T22:00:00Z",
        "source_end_exclusive_utc": "2026-05-31T00:00:00Z",
        "retrieval_finished_at_utc": "2026-05-31T00:00:00.123456Z",
    ...
    assert capability.status == "READY", capability.reason
    assert capability.source_provenance_sha256 == "a" * 64
    assert isinstance(capability.development_provenance_sha256, str)
    assert len(capability.development_provenance_sha256) == 64


def test_t0_development_root_rejects_noncanonical_bound_run_id(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database, candidate_id = _approved_candidate_database(tmp_path / "database")
    pilot, python, source = _frozen_capability_fixture(
        tmp_path / "legacy-source", monkeypatch
    )
    _configure_rolling_window_fixture(pilot)
    output_parent = tmp_path / "published-roots"
    output_parent.mkdir()
    ...
    with pytest.raises(
        development_run.DevelopmentRunError,
        match="canonical UUIDv4",
    ):
        development_run.prepare_development_root(
            pilot,
            output_parent / "development",
            python,
            source,
            database,
            candidate_id,
            research_run_id="not-a-console-campaign-id",
        )

    assert not (output_parent / "development").exists()


def test_t0_prepared_development_root_manifest_drift_fails_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (
    ...
    manifest["binding"]["research_run_id"] = str(uuid4())
    manifest_path.write_bytes(_canonical(manifest))
    manifest_path.chmod(0o400)
    development_root.chmod(0o500)
...
def test_t1_prepared_development_root_uses_prebound_research_run_id(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (
    ...
    mismatched_dir = tmp_path / "runtime" / mismatched_id
    mismatched_dir.mkdir(parents=True)

    with pytest.raises(development_run.DevelopmentRunError) as mismatched:
        development_run.prepare_development_run(
            database,
            mismatched_dir,
            candidate_id,
            capability,
            research_run_id=mismatched_id,
            now=NOW,
        )
    assert mismatched.value.message == (
        "Development root research_run_id binding mismatch"
    )
    assert list(mismatched_dir.iterdir()) == []

    run_dir = mismatched_dir.parent / research_run_id
    run_dir.mkdir()
    ...
    with get_connection(database, read_only=True) as connection:
        run = connection.execute(
            "SELECT id,candidate_id,input_snapshot_json FROM research_runs WHERE id=?",
            (research_run_id,),
        ).fetchone()
        executions = connection.execute(
            """
            SELECT scenario,sequence,status
            FROM backtest_executions WHERE research_run_id=? ORDER BY sequence
            """,
            (research_run_id,),
        ).fetchall()
    snapshot = json.loads(run["input_snapshot_json"])
    assert (run["id"], run["candidate_id"]) == (research_run_id, candidate_id)
    assert snapshot["development_root_research_run_id"] == research_run_id
    assert snapshot["development_root_manifest_sha256"] == capability.plan_sha256
    assert [(row["scenario"], row["sequence"], row["status"]) for row in executions] == [
        ("DEVELOPMENT", 1, "PENDING")
    ]
    assert snapshot["holdout"] == "SEALED_UNREAD"
    assert snapshot["holdout_stress"] == "SEALED_UNREAD"
    assert not any(
        token in path.relative_to(run_dir).as_posix().lower()
        for path in run_dir.rglob("*")
        for token in ("holdout", "stress")
    )
    assert development_root != run_dir
...
def test_t0_context_prioritizes_security_profile_then_runtime_blockers(
    ...
    capability = development_run.FrozenDevelopmentCapability(
        ...
        pair="ADA/USDT:USDT",
        root_schema=development_run.DEVELOPMENT_ROOT_SCHEMA,
    )
    ...
    duration_database, duration_candidate = _approved_candidate_database(
        tmp_path / "duration",
        holdout_days=15,
    )
    ...
    blocked_duration = development_run.research_context(
        duration_database, capability
    )
    ...
    assert blocked_duration["candidates"][0]["candidate_id"] == duration_candidate
    assert blocked_duration["candidates"][0]["status"] == "BLOCKED_PROFILE"
...
def test_t0_staged_context_never_leaks_an_unbound_latest_run(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database, candidate_id = _approved_candidate_database(tmp_path)
    bound_run_id = str(uuid4())
    unrelated_run_id = str(uuid4())
    capability = development_run.FrozenDevelopmentCapability(
        status="READY",
        reason="staged root ready",
        pair="ADA/USDT:USDT",
        development_timerange="20260601-20260731",
        root_schema=development_run.DEVELOPMENT_ROOT_SCHEMA,
        bound_candidate_id=candidate_id,
        bound_research_run_id=bound_run_id,
    )
    monkeypatch.setattr(development_run, "_require_ready", lambda _capability: None)
    with get_connection(database) as connection:
        profile_id = connection.execute(
            "SELECT research_profile_id FROM generation_runs LIMIT 1"
        ).fetchone()[0]
        connection.execute(
            """
            INSERT INTO research_runs (
                ...
                verdict,pipeline_version,input_snapshot_json,checks_json,run_dir,
                rejection_reasons_json,created_at,started_at,finished_at
            ) VALUES (?,?,?,'MANUAL','FAILED','DEVELOPMENT_BACKTEST',NULL,?,
                      '{}','{}',?,'[]',?,?,?)
            """,
            (
                unrelated_run_id,
    ...
    assert context["latest_research_run_id"] is None
