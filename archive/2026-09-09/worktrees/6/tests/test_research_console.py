...
from lab.development_run import FrozenDevelopmentCapability
from scripts import serve_research_console
...
def test_t0_console_cli_accepts_legacy_or_explicit_development_root() -> None:
    common = [
        "--database",
        "/tmp/lab.sqlite",
        "--runtime-root",
    ...
    legacy = serve_research_console.parse_args(
        [*common, "--pilot-root", "/tmp/pilot"]
    )
    assert legacy.pilot_root == Path("/tmp/pilot")
    assert legacy.development_root is None
    assert legacy.holdout_root is None

    split = serve_research_console.parse_args(
        [
            *common,
            "--development-root",
            "/tmp/development",
    ...
    assert split.pilot_root is None
    assert split.development_root == Path("/tmp/development")
    assert split.holdout_root == Path("/tmp/holdout")

    with pytest.raises(SystemExit):
        serve_research_console.parse_args(
            [
                *common,
                "--pilot-root",
                "/tmp/pilot",
...
def test_t1_explicit_development_without_holdout_stays_ready_and_sealed(
    database: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime_root = tmp_path / "runtime-split"
    development_root = tmp_path / "development"
    runtime_root.mkdir()
    development_root.mkdir()
    development_capability = FrozenDevelopmentCapability(
        status="READY",
        reason="staged Development root is frozen",
        pilot_root=development_root.resolve(),
        root_schema="freqtrade-lab-development-root-v1",
        bound_research_run_id=str(uuid4()),
    )
    monkeypatch.setattr(
        research_console,
        "freeze_development_capability",
        lambda _root, _python, _source: development_capability,
    )

    def unexpected_holdout_freeze(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("missing Holdout root must not be opened or frozen")

    monkeypatch.setattr(
        research_console,
        "freeze_holdout_capability",
        unexpected_holdout_freeze,
    )
    monkeypatch.setattr(
        research_console,
        "probe_frequi",
    ...
    try:
        ...
        assert controller.config.development_root == development_root.resolve()
        assert controller.config.holdout_root is None
        assert controller.config.legacy_combined_root is False
        assert controller.check_data_argv[2:5] == (
            "check-development-root",
            "--development-root",
            str(development_root.resolve()),
        )
        assert "--pilot-root" not in controller.check_data_argv
        preflight = controller.preflight()
        assert preflight["checks"]["development_root"]["status"] == "READY"
        assert preflight["checks"]["development_research"]["status"] == "READY"
        assert preflight["checks"]["holdout_root"] == {
            "status": "BLOCKED_DATA",
            "outside_git": False,
            "configured": False,
            "sealed": True,
            "message": "Holdout 目录未配置或身份不可用，市场值保持封存",
        }
        holdout = preflight["checks"]["holdout_continuation"]
        assert holdout["status"] == "BLOCKED_DATA"
        assert holdout["holdout"] == "SEALED_UNREAD"
        assert holdout["holdout_stress"] == "SEALED_UNREAD"
    finally:
        server.research_console_controller.shutdown()
        server.server_close()


def test_t1_explicit_holdout_is_frozen_against_the_development_capability(
    database: Path,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime_root = tmp_path / "runtime-split-with-holdout"
    development_root = tmp_path / "development-with-holdout"
    holdout_root = tmp_path / "holdout"
    for path in (runtime_root, development_root, holdout_root):
        path.mkdir()
    development_capability = FrozenDevelopmentCapability(
        status="READY",
        reason="staged Development root is frozen",
    ...
    def freeze_holdout(
        root: Path,
        _python: Optional[Path],
        _source: Optional[Path],
        *,
        development_capability: FrozenDevelopmentCapability,
    ) -> Any:
        captured["root"] = root
        captured["development"] = development_capability
        return research_console.FrozenHoldoutCapability(
            status="BLOCKED_DATA",
            reason="test Holdout remains sealed",
    ...
    try:
        controller = server.research_console_controller
        assert controller.config.holdout_root == holdout_root.resolve()
        assert captured == {
...
def test_t0_split_holdout_never_authorizes_an_unbound_research_run() -> None:
    controller = object.__new__(research_console.ResearchConsoleController)
    bound_run_id = str(uuid4())
    controller._holdout_capability = research_console.FrozenHoldoutCapability(
        status="READY",
        reason="split Holdout ready",
        separate_root=True,
        bound_research_run_id=bound_run_id,
    )
    controller._lock = threading.RLock()
    controller._active = None
    controller._restart_confirmation_required = False
    controller._closed = False
    controller._shutting_down = False
    controller._release_root = None
    controller._runtime_paths_unchanged = lambda: True
    controller._campaign_directory = lambda *_args, **_kwargs: Path("/tmp/test")

    result = controller._decorate_research_run(
        {
            "research_run_id": str(uuid4()),
            "status": "PENDING",
            "authorization": {
                "can_authorize": True,
                "reason": "database row is otherwise eligible",
            },
        }
    )
    ...
    assert result["authorization"] == {
        "can_authorize": False,
        "reason": "ResearchRun 不属于当前冻结的 Holdout 根",
    }
