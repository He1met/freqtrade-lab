...
from lab.development_run import (
    ...
    DEVELOPMENT_ROOT_MANIFEST,
    DEVELOPMENT_ROOT_SCHEMA,
    EXPECTED_GATE,
    SUPPORTED_FREQTRADE_COMMIT,
    DevelopmentRunError,
    FrozenDevelopmentCapability,
    _bound_candidate as _bound_development_candidate,
    _profile_binding_sha256 as _development_profile_binding_sha256,
    _profile_gate as _development_profile_gate,
    _schema_v1 as _development_schema_v1,
    freeze_development_capability,
    prepare_development_root as _prepare_legacy_development_root,
)
from lab.holdout_run import (
    HOLDOUT_ROOT_MANIFEST,
    HOLDOUT_ROOT_SCHEMA,
    HoldoutRunError,
    _eligible_row as _eligible_holdout_row,
    _freeze_staged_holdout_root,
    _profile_binding_sha256 as _holdout_profile_binding_sha256,
)
...
def _series_window_contract(
    value: Any, label: str, expected_days: int
) -> dict[str, Any]:
    search_start, search_stop = timerange(value, label)
    if search_stop - search_start != timedelta(days=expected_days):
        raise PilotError(
            f"{label} window must span exactly {expected_days} days"
        )
    starts = {
...
def _search_window_contract(value: Any) -> dict[str, Any]:
    return _series_window_contract(value, "Search", 30)
...
def _load_search_source(
    ...
    *,
    required_timerange: Optional[str] = FROZEN_SEARCH_TIMERANGE,
    require_retrieval_finish: bool = False,
    purpose: str = "Search",
    required_pair: Optional[str] = "XRP/USDT:USDT",
    ...
    direct_development = required_timerange is None and purpose == "Development"
    source_pair = source.get("pair") if isinstance(source, dict) else None
    source_instrument = (
        source.get("instrument_id") if isinstance(source, dict) else None
    )
    if (
        ...
        or not isinstance(source_pair, str)
        or not source_pair
        or (required_pair is not None and source_pair != required_pair)
        or not isinstance(source_instrument, str)
        or not source_instrument
        ...
        raise PilotError("source must be one complete singular public acquisition")
    if direct_development and set(contract) != {
        "data_dir",
        "market_snapshot",
        "leverage_tiers",
        "config",
        ...
        raise PilotError(
            "direct Development source contract must not contain Holdout fields"
        )
    source_development_timerange = contract.get("development_timerange")
    source_development_start, source_development_stop = timerange(
        source_development_timerange, "source Development"
    )
    selected_timerange = (
        source_development_timerange
        if required_timerange is None
        else required_timerange
    )
    selected_start, selected_stop = timerange(
        selected_timerange, f"{purpose} Development"
    )
    if (
        source_development_start > selected_start
        or source_development_stop < selected_stop
    ):
        raise PilotError(f"source acquisition does not cover {purpose} contract")
    ...
    for name, expected_sha in (
        ...
        if (
            name == receipt_name
            and not require_tracked_receipt
            and digest(data) == expected_sha
        ):
            continue
        if (
    ...
    try:
        ...
        if pairs != (source_pair,):
            raise ResearchCandidateError("config must bind the source pair")
    ...
    if (
        ...
        or receipt.get("pair") != source_pair
        or receipt.get("instrument_id") != source_instrument
    ):
    ...
    if require_retrieval_finish:
        retrieval_finished = _source_timestamp(
            receipt.get("finished_at_utc"), "source retrieval finish"
        )
    if (
        source_start > selected_start - timedelta(hours=2)
        or source_stop < selected_stop
        or (
            require_retrieval_finish
            and retrieval_finished is not None
            and retrieval_finished < selected_stop
        )
    ):
        raise PilotError(f"source acquisition does not cover closed {purpose}")
    exact_rows: Optional[dict[str, int]] = None
    if direct_development:
        if set(window) != {
            ...
            raise PilotError(
                "direct Development data window must not contain Holdout fields"
            )
        if (
            window.get("schema")
            != "freqtrade-lab-okx-development-window-v1"
            or _source_timestamp(
                window.get("development_start_utc"),
                "source Development start",
            )
            != selected_start
            or source_start != selected_start - timedelta(hours=2)
            or source_stop != selected_stop
            or window.get("startup_candles_required") != 20
        ):
            raise PilotError(
                "source acquisition does not match the exact closed Development window"
            )
        series = receipt.get("series")
        if not isinstance(series, dict) or set(series) != set(SEARCH_SERIES_STEPS):
            raise PilotError("source Development series receipt is incomplete")
        starts = {
            "futures_5m": source_start,
            "mark_1h": source_start,
            "funding_history": selected_start,
        }
        exact_rows = {}
        for name, step in SEARCH_SERIES_STEPS.items():
            record = series.get(name)
            expected_rows = int((selected_stop - starts[name]) / step)
            expected_end = selected_stop - step
            if (
                not isinstance(record, dict)
                or isinstance(record.get("rows"), bool)
                or record.get("rows") != expected_rows
                or _source_timestamp(
                    record.get("start_utc"), f"{name} receipt start"
                )
                != starts[name]
                or _source_timestamp(
                    record.get("end_utc"), f"{name} receipt end"
                )
                != expected_end
                or record.get("duplicates") != 0
                or (
                    name != "funding_history"
                    and (
                        record.get("missing_intervals") != 0
                        or record.get("unclosed_candles") != 0
                    )
                )
                or (
                    name == "funding_history"
                    and record.get("outside_window") != 0
                )
            ):
                raise PilotError(
                    f"source Development {name} receipt is not exact and closed"
                )
            exact_rows[name] = expected_rows
    return {
        ...
        "development_timerange": source_development_timerange,
        "retrieval_finished_at_utc": (
            None
            if retrieval_finished is None
            else retrieval_finished.isoformat().replace("+00:00", "Z")
        ),
        "source_start_utc": source_start.isoformat().replace("+00:00", "Z"),
        "source_end_exclusive_utc": source_stop.isoformat().replace(
            "+00:00", "Z"
        ),
        "fully_closed_at_fetch": window.get("fully_closed_at_fetch") is True,
        "rows": exact_rows,
    }
...
def _verify_search_output_dates(
    ...
    *,
    _expected_days: int = 30,
    _label: str = "Search-only",
    _expected_rows: Optional[Mapping[str, int]] = None,
) -> dict[str, int]:
    try:
        import pyarrow
    ...
    window = _series_window_contract(
        search_timerange, _label.removesuffix("-only"), _expected_days
    )
    if _expected_rows is not None and dict(_expected_rows) != window["rows"]:
        raise PilotError(f"{_label} row receipt disagrees with its window")
    ...
    for series, relative_name in data_names.items():
        try:
            ...
            if tuple(table.column_names) != expected_columns:
                raise PilotError(
                    f"{_label} {series} does not have the exact Freqtrade OHLCV schema"
                )
            dates = table.column("date")
            if dates.null_count:
                raise PilotError(f"{_label} {series} timestamps contain nulls")
            for column_name in expected_columns[1:]:
                column = table.column(column_name)
                if not pyarrow.types.is_floating(column.type):
                    raise PilotError(
                        f"{_label} {series} {column_name} values are invalid"
                    )
            ...
            raise PilotError(f"{_label} {series} is not a readable Feather series") from exc
        if any(
            not isinstance(item, datetime) or item.utcoffset() != timedelta(0)
            for item in raw_values
        ):
            raise PilotError(f"{_label} {series} timestamps are not UTC")
        ...
        if (
            ...
            raise PilotError(f"{_label} {series} output is not contiguous")
    return dict(window["rows"])


def _verify_development_output_dates(
    data_root: Path,
    data_names: Mapping[str, str],
    development_timerange: str,
    expected_rows: Mapping[str, int],
) -> dict[str, int]:
    return _verify_search_output_dates(
        data_root,
        data_names,
        development_timerange,
        _expected_days=60,
...
def _canonical_research_run_id(value: Optional[str]) -> str:
    run_id = str(uuid4()) if value is None else value
    try:
        parsed = UUID(run_id)
    except (AttributeError, TypeError, ValueError) as exc:
        raise PilotError("research_run_id must be a canonical UUIDv4") from exc
    if parsed.version != 4 or str(parsed) != run_id:
        raise PilotError("research_run_id must be a canonical UUIDv4")
    return run_id
...
def _outside_git(path: Path, label: str) -> None:
    ...
    while True:
        if (current / ".git").exists() or (current / ".git").is_symlink():
            raise PilotError(f"{label} must stay outside every Git worktree")
        if current.parent == current:
            return
        current = current.parent


def _prepare_direct_development_root(
    source_root: Path,
    output_root: Path,
    ...
    *,
    ...
    trusted_source_provenance_sha256: Optional[str],
    trusted_source_receipt_sha256: Optional[str],
) -> dict[str, Any]:
    """Publish 60 closed Development days without acquiring future Holdout."""
    if (
        trusted_source_provenance_sha256 is None
        or trusted_source_receipt_sha256 is None
    ):
        raise PilotError("trusted Development source hashes are required")
    raw_source = source_root.expanduser()
    if raw_source.is_symlink():
        raise PilotError("Development source root must not be a symlink")
    source = raw_source.resolve(strict=True)
    acquisition = source / ACQUISITION
    if not acquisition.is_dir():
        acquisition = source
    if acquisition.is_symlink() or not (
        acquisition / "retained-data-provenance.json"
    ).is_file():
        raise PilotError("Development source acquisition is unavailable")
    _outside_git(source, "Development source root")

    frozen = _load_search_source(
        acquisition,
        trusted_source_provenance_sha256,
        trusted_source_receipt_sha256,
        required_timerange=None,
        require_retrieval_finish=True,
        purpose="Development",
        required_pair=None,
        require_tracked_receipt=False,
    )
    development_timerange = frozen["development_timerange"]
    development_start, development_stop = timerange(
        development_timerange, "Development"
    )
    if development_stop - development_start != timedelta(days=60):
        raise PilotError("Development source must bind exactly 60 closed days")
    run_id = _canonical_research_run_id(research_run_id)
    profile_capability = FrozenDevelopmentCapability(
        status="READY",
        reason="direct Development source identity is verified",
        pair=str(frozen["source"]["pair"]),
        root_schema=DEVELOPMENT_ROOT_SCHEMA,
    )
    try:
        connection = get_connection(database_path, must_exist=True, read_only=True)
    except (OSError, sqlite3.Error) as exc:
        raise PilotError("database is unavailable") from exc
    try:
        with closing(connection):
            connection.execute("BEGIN")
            _development_schema_v1(connection)
            candidate = _bound_development_candidate(connection, candidate_id)
            _development_profile_gate(candidate, profile_capability)
            profile_id = str(candidate["research_profile_id"])
            binding = {
                "research_run_id": run_id,
                "candidate_id": str(candidate["id"]),
                "generation_run_id": str(candidate["generation_run_id"]),
                "class_name": str(candidate["class_name"]),
                "candidate_code_sha256": str(candidate["code_sha256"]),
                "research_profile_id": profile_id,
                "profile_binding_sha256": _development_profile_binding_sha256(
    ...
    output_parent = expanded_output.parent.resolve(strict=True)
    output = output_parent / expanded_output.name
    _outside_git(output_parent, "Development output root")
    if output.exists() or output.is_symlink():
    ...
    staging = Path(
        tempfile.mkdtemp(prefix=".development-root.preparing-", dir=output_parent)
    )
    staging.chmod(0o700)
    published = False
    try:
        data_root = staging / "data" / "okx"
        data_root.mkdir(parents=True)
        rows = frozen.get("rows")
        if not isinstance(rows, dict) or set(rows) != set(SEARCH_SERIES_STEPS):
            raise PilotError("source Development row receipt is unavailable")
        ...
        output_data_sha256: dict[str, str] = {}
        for relative_name in frozen["data_names"].values():
            source_path = safe_file(
                acquisition / "data" / "okx",
                relative_name,
                "source Development market data",
            )
            data = source_path.read_bytes()
            if digest(data) != frozen["data_sha256"][relative_name]:
                raise PilotError("source Development market data changed")
            path = data_root / relative_name
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(data)
            manifest_name = f"data/okx/{relative_name}"
            output_data_sha256[manifest_name] = digest(data)
            files[manifest_name] = {
                "bytes": len(data),
                "sha256": output_data_sha256[manifest_name],
                "role": "market_data",
            }
        rows = _verify_development_output_dates(
            data_root,
            frozen["data_names"],
            str(development_timerange),
            rows,
        )
        manifest = {
            "schema": DEVELOPMENT_ROOT_SCHEMA,
            "development_timerange": development_timerange,
            "gate": {"version": DEVELOPMENT_GATE_VERSION, **EXPECTED_GATE},
            "source": {
                key: frozen["source"][key]
                for key in ("host", "authentication", "pair", "instrument_id")
            },
            "freqtrade": {
                "version": "2026.7",
                "tag": "2026.7",
                "commit": SUPPORTED_FREQTRADE_COMMIT,
                "tree": SUPPORTED_FREQTRADE_TREE,
                "dependencies": dict(RUNNER_DEPENDENCIES),
            },
            "source_receipts": {
                "source_provenance_sha256": trusted_source_provenance_sha256,
                "retrieval_receipt_sha256": trusted_source_receipt_sha256,
                "source_start_utc": frozen["source_start_utc"],
                "source_end_exclusive_utc": frozen[
                    "source_end_exclusive_utc"
                ],
                "retrieval_finished_at_utc": frozen[
                    "retrieval_finished_at_utc"
                ],
                "fully_closed_at_fetch": frozen["fully_closed_at_fetch"],
            },
            "binding": binding,
            "files": dict(sorted(files.items())),
        }
        manifest_bytes = canonical(manifest)
        (staging / DEVELOPMENT_ROOT_MANIFEST).write_bytes(manifest_bytes)
        for path in staging.rglob("*"):
            path.chmod(0o500 if path.is_dir() else 0o400)
        staging.chmod(0o500)
        capability = freeze_development_capability(
            staging, freqtrade_python, freqtrade_source
        )
        if capability.status != "READY":
            raise PilotError("published Development root contract did not verify")
        try:
            _publish_directory_exclusive(staging, output)
        except ResearchCandidateError as exc:
            raise PilotError(
                "Development output root was claimed concurrently"
            ) from exc
        published = True
        return {
            "status": "DEVELOPMENT_ROOT_READY",
            "schema": DEVELOPMENT_ROOT_SCHEMA,
            "development_timerange": development_timerange,
            ...
            "research_profile_id": binding["research_profile_id"],
            "trusted_source_provenance_sha256": (
                trusted_source_provenance_sha256
            ),
...
def prepare_development_root(
    source_pilot_root: Path,
    output_root: Path,
    freqtrade_python: Path,
    freqtrade_source: Path,
    database_path: Path,
    ...
    *,
    ...
    trusted_source_provenance_sha256: Optional[str] = None,
    trusted_source_receipt_sha256: Optional[str] = None,
) -> dict[str, Any]:
    """Prefer a direct closed Development acquisition; retain legacy adapter."""
    source = source_pilot_root.expanduser()
    if (source / PLAN).is_file():
        try:
            return _prepare_legacy_development_root(
                source,
                output_root,
                freqtrade_python,
                freqtrade_source,
            ...
            raise PilotError(exc.message) from exc
    return _prepare_direct_development_root(
        source,
        output_root,
        freqtrade_python,
        freqtrade_source,
        ...
        candidate_id,
        research_run_id=research_run_id,
        trusted_source_provenance_sha256=trusted_source_provenance_sha256,
        trusted_source_receipt_sha256=trusted_source_receipt_sha256,
    )


def _trim_holdout_view(
    data_root: Path,
    data_names: Mapping[str, str],
    holdout_timerange: str,
) -> dict[str, dict[str, Any]]:
    """Retain only two startup hours plus the frozen 30-day Holdout."""
    try:
        import pyarrow
        import pyarrow.compute as pc
        import pyarrow.feather as feather
    ...
    if pyarrow.__version__ != RUNNER_DEPENDENCIES["pyarrow"]:
        raise PilotError("exact PyArrow 25.0.0 is required")
    window = _search_window_contract(holdout_timerange)
    for series, relative_name in data_names.items():
        path = data_root / relative_name
        try:
            table = feather.read_table(path)
            dates = table.column("date")
            start = pyarrow.scalar(window["starts"][series], type=dates.type)
            stop = pyarrow.scalar(window["search_stop"], type=dates.type)
            retained = table.filter(
                pc.and_(pc.greater_equal(dates, start), pc.less(dates, stop))
            )
            feather.write_feather(retained, path, compression="uncompressed")
        except Exception as exc:
            raise PilotError(f"Holdout-only {series} slice failed") from exc
    rows = _verify_search_output_dates(data_root, data_names, holdout_timerange)
    return {
        relative_name: {
            "bytes": (data_root / relative_name).stat().st_size,
            "sha256": digest((data_root / relative_name).read_bytes()),
            "rows": rows[series],
        }
        for series, relative_name in data_names.items()
    }
...
def prepare_holdout_root(
    ...
    """Publish one immutable future Holdout-only root for one passed Run."""
    if any(
        re.fullmatch(r"[0-9a-f]{64}", value) is None
        for value in (trusted_provenance_sha256, trusted_receipt_sha256)
    ):
        raise PilotError("trusted source SHA-256 values are invalid")
    raw_source = source_pilot_root.expanduser()
    if raw_source.is_symlink():
        raise PilotError("source Pilot root must not be a symlink")
    source_root = raw_source.resolve(strict=True)
    current = source_root
    while True:
        if (current / ".git").exists() or (current / ".git").is_symlink():
            raise PilotError("source Pilot root must stay outside every Git worktree")
        if current.parent == current:
            break
        current = current.parent
    capability = freeze_development_capability(
    ...
    if (
        ...
        or capability.root_schema != DEVELOPMENT_ROOT_SCHEMA
        or capability.pilot_root is None
        or capability.bound_research_run_id != research_run_id
    ):
        raise PilotError("bound Development root is unavailable")

    try:
        with closing(
            get_connection(database_path, must_exist=True, read_only=True)
        ) as connection:
            connection.execute("BEGIN")
            row, parsed, snapshot = _eligible_holdout_row(
                connection, research_run_id, parse_artifact=True
            )
            profile_binding_sha256 = _holdout_profile_binding_sha256(
                connection, str(row["research_profile_id"])
            )
            connection.rollback()
    except (HoldoutRunError, OSError, sqlite3.Error) as exc:
        raise PilotError("passed Development run is unavailable") from exc
    if parsed is None:
        raise PilotError("Development Artifact is unavailable")
    future = snapshot.get("future_holdout")
    if (
        not isinstance(future, dict)
        or capability.bound_candidate_id != row["candidate_id"]
        or capability.bound_generation_run_id != row["generation_run_id"]
        or capability.bound_candidate_sha256 != row["code_sha256"]
        or capability.bound_profile_id != row["research_profile_id"]
        or capability.bound_profile_sha256 != profile_binding_sha256
        or capability.plan_sha256
        != snapshot.get("development_root_manifest_sha256")
    ):
        raise PilotError("Development root binding disagrees with the passed Run")
    holdout_timerange = future.get("timerange")
    holdout_start, holdout_stop = timerange(holdout_timerange, "future Holdout")
    run_dir = Path(str(row["run_dir"])).resolve(strict=True)
    development_input_manifest = run_dir / "development-input" / "manifest.json"
    development_input_manifest_sha256 = digest(
        development_input_manifest.read_bytes()
    )

    plan = load_plan(source_root)
    if (
        plan.get("holdout_timerange") != holdout_timerange
        or not math.isclose(
            float(plan.get("stress_fee_multiplier")),
            float(row["stress_fee_multiplier"]),
            rel_tol=0.0,
            abs_tol=1e-15,
        )
    ):
        raise PilotError("source Pilot does not target the frozen future Holdout")
    verified_public = verify_data(source_root, plan)
    if (
        verified_public.get("provenance_sha256") != trusted_provenance_sha256
        or verified_public.get("retrieval_receipt_sha256")
        != trusted_receipt_sha256
    ):
        raise PilotError("trusted source SHA-256 values do not match the acquisition")

    acquisition = source_root / ACQUISITION
    provenance, provenance_bytes = load_json(
        acquisition / "retained-data-provenance.json", "source data provenance"
    )
    source = provenance.get("source")
    contract = provenance.get("contract")
    if not isinstance(source, dict) or not isinstance(contract, dict):
        raise PilotError("source data provenance identity is invalid")
    receipt_name = source.get("retrieval_receipt")
    receipt, receipt_bytes = load_json(
        safe_file(acquisition, receipt_name, "source retrieval receipt"),
        "source retrieval receipt",
    )
    data_window = receipt.get("data_window")
    if (
        digest(provenance_bytes) != trusted_provenance_sha256
        or digest(receipt_bytes) != trusted_receipt_sha256
        or source.get("host") != "www.okx.com"
        or source.get("authentication") != "none"
        or source.get("pair") != capability.pair
        or source.get("instrument_id") != capability.instrument_id
        or receipt.get("host") != "www.okx.com"
        or receipt.get("authentication") != "none"
        or receipt.get("pair") != capability.pair
        or receipt.get("instrument_id") != capability.instrument_id
        or not isinstance(data_window, dict)
        or data_window.get("fully_closed_at_fetch") is not True
    ):
        raise PilotError("source retrieval identity/completeness receipt mismatch")
    source_start = _source_timestamp(data_window.get("start_utc"), "source start")
    source_stop = _source_timestamp(
        data_window.get("end_exclusive_utc"), "source stop"
    )
    retrieval_finished = _source_timestamp(
        receipt.get("finished_at_utc"), "source retrieval finish"
    )
    if (
        source_start > holdout_start - timedelta(hours=2)
        or source_stop < holdout_stop
        or retrieval_finished < holdout_stop
    ):
        raise PilotError("source acquisition does not prove a closed future Holdout")
    ...
    try:
        ...
        verified = _verify_data_provenance(
            provenance,
            scenario="HOLDOUT",
            timerange=str(holdout_timerange),
            pair=str(capability.pair),
            data_dir=acquisition / "data" / "okx",
            market_snapshot=safe_file(
                acquisition, contract.get("market_snapshot"), "source market snapshot"
            ),
            leverage_tiers=safe_file(
                acquisition, contract.get("leverage_tiers"), "source leverage tiers"
            ),
        )
    except OfflineBacktestError as exc:
        raise PilotError(str(exc)) from exc
    data_names = _search_data_names(capability.pair)
    if set(verified["data_sha256"]) != set(data_names.values()):
        raise PilotError("source acquisition must contain exact 5m/mark/funding data")
    controls = {
        "config.json": safe_file(
            acquisition, contract.get("config"), "source config"
        ).read_bytes(),
        "market_snapshot.json": safe_file(
            acquisition, contract.get("market_snapshot"), "source market snapshot"
        ).read_bytes(),
        "isolated_tiers_snapshot.json": safe_file(
            acquisition, contract.get("leverage_tiers"), "source leverage tiers"
        ).read_bytes(),
    }
    if digest(controls["config.json"]) != capability.config_sha256:
        raise PilotError("future Holdout config drifted from Development")

    expanded_output = output_root.expanduser()
    if expanded_output.name in {"", ".", ".."}:
        raise PilotError("Holdout output root must name one new directory")
    output_parent = expanded_output.parent.resolve(strict=True)
    output = output_parent / expanded_output.name
    try:
        output.relative_to(ROOT.resolve(strict=True))
        ...
        raise PilotError("Holdout output root must stay outside Git")
    current = output_parent
    while True:
        if (current / ".git").exists() or (current / ".git").is_symlink():
            raise PilotError("Holdout output root must stay outside every Git worktree")
        if current.parent == current:
            break
        current = current.parent
    if output.exists() or output.is_symlink():
        raise PilotError("Holdout output root already exists; replay is forbidden")
    roots = (source_root, capability.pilot_root, run_dir)
    if any(
        root == output_parent or root in output.parents or output in root.parents
        for root in roots
    ):
        raise PilotError("source, Development, Run, and Holdout roots must be independent")

    staging = Path(tempfile.mkdtemp(prefix=".holdout-root-", dir=output_parent))
    staging.chmod(0o700)
    published = False
    try:
        data_root = staging / "data" / "okx"
        ...
        _create_scenario_data_view(
            acquisition / "data" / "okx",
            data_root,
            str(holdout_timerange),
            verified["data_sha256"],
        )
        phase_files = _trim_holdout_view(
            data_root, data_names, str(holdout_timerange)
        )
        files: dict[str, dict[str, Any]] = {}
        ...
        for relative_name, record in phase_files.items():
            files[f"data/okx/{relative_name}"] = {
                "bytes": record["bytes"],
                "sha256": record["sha256"],
                "role": "market_data",
            }
        manifest = {
            "schema": HOLDOUT_ROOT_SCHEMA,
            "binding": {
                "research_run_id": research_run_id,
                ...
                "development_root_manifest_sha256": capability.plan_sha256,
                "development_input_manifest_sha256": (
                    development_input_manifest_sha256
                ),
            ...
            "policy": {
                "future_window": "NEXT_UTC_DAY_AFTER_DEVELOPMENT_PASS_V1",
                "max_open_count": 1,
                "retry_after_open": False,
                "tune_after_result": False,
                "stress_reuses_holdout_bytes": True,
            },
            "source": {
                "host": "www.okx.com",
                "authentication": "none",
            ...
            "source_receipts": {
                "source_provenance_sha256": trusted_provenance_sha256,
                "retrieval_receipt_sha256": trusted_receipt_sha256,
                "retrieval_finished_at_utc": retrieval_finished.isoformat().replace(
                    "+00:00", "Z"
                ),
                "source_start_utc": source_start.isoformat().replace("+00:00", "Z"),
                "source_end_exclusive_utc": source_stop.isoformat().replace(
        ...
        manifest_bytes = canonical(manifest)
        (staging / HOLDOUT_ROOT_MANIFEST).write_bytes(manifest_bytes)
        for path in staging.rglob("*"):
            path.chmod(0o500 if path.is_dir() else 0o400)
        staging.chmod(0o500)
        frozen_holdout = _freeze_staged_holdout_root(staging, capability)
        if frozen_holdout.status != "READY":
            raise PilotError("published Holdout root contract did not verify")
        try:
            _publish_directory_exclusive(staging, output)
        except ResearchCandidateError as exc:
            raise PilotError(str(exc)) from exc
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    check_development = commands.add_parser(
        ...
        help="verify one immutable Development-only root without opening Holdout",
    )
    check_development.add_argument("--development-root", required=True, type=Path)
    check_development.add_argument("--freqtrade-python", type=Path)
    check_development.add_argument("--freqtrade-source", type=Path)
    ...
    prepare_development = commands.add_parser(
        "prepare-development-root",
        help="publish one immutable Development-only root from a verified Pilot",
    )
    prepare_development.add_argument(
        "--source-root",
        "--source-pilot-root",
        dest="source_pilot_root",
        required=True,
        type=Path,
        help=(
            "reviewed source containing acquisition/; the legacy option name "
            "remains an alias"
        ),
    )
    prepare_development.add_argument("--source-provenance-sha256", required=True)
    prepare_development.add_argument("--source-receipt-sha256", required=True)
    prepare_development.add_argument("--output-root", required=True, type=Path)
    prepare_development.add_argument("--freqtrade-python", required=True, type=Path)
    prepare_development.add_argument("--freqtrade-source", required=True, type=Path)
    prepare_development.add_argument("--database", required=True, type=Path)
    prepare_development.add_argument("--candidate-id", required=True)
    prepare_development.add_argument("--research-run-id")
    prepare_holdout = commands.add_parser(
        "prepare-holdout-root",
        help="publish one immutable future Holdout-only root for a passed Run",
    )
    prepare_holdout.add_argument("--source-pilot-root", required=True, type=Path)
    prepare_holdout.add_argument("--source-provenance-sha256", required=True)
    prepare_holdout.add_argument("--source-receipt-sha256", required=True)
    prepare_holdout.add_argument("--output-root", required=True, type=Path)
    prepare_holdout.add_argument("--development-root", required=True, type=Path)
    prepare_holdout.add_argument("--freqtrade-python", required=True, type=Path)
    prepare_holdout.add_argument("--freqtrade-source", required=True, type=Path)
    prepare_holdout.add_argument("--database", required=True, type=Path)
    prepare_holdout.add_argument("--research-run-id", required=True)
    return parser.parse_args(argv)


def main(argv: Optional[Sequence[str]] = None) -> int:
    args = parse_args(argv)
    if args.command == "check-development-root":
        capability = freeze_development_capability(
        ...
        payload = {
            **capability.public(),
            ...
            "status": (
                "DATA_READY" if capability.status == "READY" else "BLOCKED_DATA"
            ),
        }
        print(
            json.dumps(
        ...
        return 0 if capability.status == "READY" else 3
    if args.command == "prepare-holdout-root":
        result = prepare_holdout_root(
            args.source_pilot_root,
            args.output_root,
            args.development_root,
            args.freqtrade_python,
            args.freqtrade_source,
        ...
        return 0
    if args.command == "prepare-development-root":
        try:
            result = prepare_development_root(
                args.source_pilot_root,
                args.output_root,
                args.freqtrade_python,
                args.freqtrade_source,
                ...
                trusted_source_receipt_sha256=args.source_receipt_sha256,
            )
        except DevelopmentRunError as exc:
            raise PilotError(exc.message) from exc
        print(json.dumps(result, ensure_ascii=True, separators=(",", ":"), sort_keys=True))
