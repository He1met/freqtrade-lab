...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    root_group.add_argument(
        ...
        type=Path,
        help=(
            "legacy alias for Development root; without --holdout-root it keeps "
            "the legacy combined-root behavior"
        ),
    )
    root_group.add_argument(
        "--development-root",
        type=Path,
        help="startup-frozen Development-only root outside Git",
    )
    parser.add_argument(
        "--holdout-root",
        type=Path,
        help=(
            "optional startup-frozen independent Holdout root outside Git; "
            "omission with --development-root keeps Holdout sealed and blocked"
        ),
    )
    parser.add_argument(
        "--search-root",
